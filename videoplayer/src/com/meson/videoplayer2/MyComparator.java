...
public class MyComparator implements Comparator<File> {
	...
	public int compare(File f1, File f2) {
		...
		case NAME_ASCEND:
			if (f1.isDirectory() && f2.isFile())
				...
				/*
				 * result = ustrcasecmp(returnString(f1.getName(), 0),
				 * returnString(f2.getName(), 0)); if(result > 0) result = 1;
		...
		case TYPE_ASCEND:
			if (f1.isDirectory() && f2.isFile())
				...
				String r_suf = r_name.substring(r_name.lastIndexOf(".") + 1,
						r_name.length()).toLowerCase();
				String l_suf = l_name.substring(l_name.lastIndexOf(".") + 1,
						l_name.length()).toLowerCase();
				if (r_suf.equals(l_suf)) {
					result = ustrcasecmp(returnString(r_name, 0),
							returnString(l_name, 0));
					if (result > 0)
						result = 1;
					else if (result < 0)
						result = -1;
				} else {
					result = ustrcasecmp(returnString(r_suf, 0),
							returnString(l_suf, 0));
					if (result > 0)
						result = 1;
					else if (result < 0)
						result = -1;
	...
	int ustrcharcmp(String r_str, String l_str) {
		...
		if ((str1_offset < 1) || (str2_offset < 1) || (str1_len < 1)
				|| (str2_len < 1))
			return result;
	...
	private int ustrcasecmp(String r_str, String l_str) {
		...
		while (Character.toLowerCase(r_str.charAt(offset1)) == Character
				.toLowerCase(l_str.charAt(offset2))) {
			if (ustrcheckchar(returnString(r_str, offset1),
					returnString(l_str, offset2)) == 0) {
				result = ustrcharcmp(returnString(r_str, offset1),
						returnString(l_str, offset2));
				if (result == 0) {
		...
		if (ustrcheckchar(returnString(r_str, offset1),
			...
			if (result == 0) {
				offset1 = offset1 + r_offset;
				offset2 = offset2 + l_offset;
				result = ustrcasecmp(returnString(r_str, offset1),
						returnString(l_str, offset2));
				return result;
			} else
				return result;
			...
			result = Character.toLowerCase(r_str.charAt(offset1))
					- Character.toLowerCase(l_str.charAt(offset2));
			return result;
		}
	}
