...
public class ErrorInfo {
	...
	public String getErrorInfo(int errID, String path) {
		...
		case ErrorInfo.PLAYER_UNSUPPORT:
			errStr = mContext.getResources()
					.getString(R.string.unsupport_media);// "Unsupport Media";
			break;
		case ErrorInfo.PLAYER_UNSUPPORT_VIDEO:
			errStr = mContext.getResources().getString(
					R.string.unsupport_video_format);// "Unsupport Video format";
			break;
		case ErrorInfo.PLAYER_UNSUPPORT_AUDIO:
			errStr = mContext.getResources().getString(
					R.string.unsupport_audio_format);// "Unsupport Audio format";
			break;
		case ErrorInfo.FFMPEG_OPEN_FAILED:
			errStr = mContext.getResources().getString(R.string.open_file)
					+ path + mContext.getResources().getString(R.string.failed);// "Open file ( "+PlayList.getinstance().getcur()+" ) failed";
			break;
		case ErrorInfo.FFMPEG_PARSE_FAILED:
			errStr = mContext.getResources().getString(R.string.parser_file)
					+ path + mContext.getResources().getString(R.string.failed);// "Parser file ( "+PlayList.getinstance().getcur()+" ) failed";
			break;
		case ErrorInfo.DECODER_INIT_FAILED:
			errStr = mContext.getResources().getString(
					R.string.decode_init_failed);// "Decode Init failed";
			break;
		case ErrorInfo.PLAYER_NO_VIDEO:
			errStr = mContext.getResources().getString(
					R.string.file_have_no_video);// "file have no video";
			break;
		case ErrorInfo.PLAYER_NO_AUDIO:
			errStr = mContext.getResources().getString(
					R.string.file_have_no_audio);// "file have no audio";
			break;
		case ErrorInfo.PLAYER_SET_NOVIDEO:
			errStr = mContext.getResources().getString(
					R.string.set_playback_without_video);// "set playback without video";
			break;
		case ErrorInfo.PLAYER_SET_NOAUDIO:
			errStr = mContext.getResources().getString(
					R.string.set_playback_without_audio);// "set playback without audio";
			break;
		case ErrorInfo.DIVX_AUTHOR_ERR:
			errStr = mContext.getResources().getString(
					R.string.not_authorized_to_play);// "This player is not authorized to play this video";
			break;
		case ErrorInfo.PLAYER_UNSUPPORT_VCODEC:
			errStr = mContext.getResources().getString(
					R.string.unsupport_video_codec);// "Unsupport Video codec";
			break;
		case ErrorInfo.PLAYER_FFFB_UNSUPPORT:
			errStr = mContext.getResources()
					.getString(R.string.unsupport_ff_fb);// "Unsupport ff/fb";
			break;
		case ErrorInfo.PLAYER_SEEK_OVERSPILL:
			errStr = mContext.getResources().getString(
					R.string.seek_out_of_range);// "Seek out of range";
			break;
		default:
