...
FFDemux::FFDemux() :
		src(NULL), mFormatContext(NULL), videoIndex(-1), audioIndex(-1), subtitleIndex(
				-1), videoStreamIndex(-1), audioStreamIndex(-1), subtitleStreamIndex(
				-1), mStatus(0), currentTrack(-1), mSampleTimeUs(0), mPTSPopulator(
				NULL), mStartTimeUs(0) {
...
AVFormatContext* FFDemux::openAVFormatContext(FFIO *fio) {
	...
	int res = avformat_open_input(&context, NULL, // need to pass a filename
...
status_t FFDemux::post_init() {
	...
	mTracks.clear();
	ret = av_find_best_stream(mFormatContext, AVMEDIA_TYPE_VIDEO, -1, -1, NULL,
			0);
	if (ret < 0) {
		ALOGW(
				"Could not find %s stream in input file\n", av_get_media_type_string(AVMEDIA_TYPE_VIDEO));
	} else {
	...
	ret = av_find_best_stream(mFormatContext, AVMEDIA_TYPE_AUDIO, -1, -1, NULL,
			0);
	if (ret < 0) {
		ALOGW(
	...
	ret = av_find_best_stream(mFormatContext, AVMEDIA_TYPE_SUBTITLE, -1, -1,
			NULL, 0);
	if (ret < 0) {
		ALOGW(
	...
	if (videoIndex == -1 && audioIndex == -1) {
		ALOGE("What a f**king stream, no audio/video\n");
		goto exit;
	}
	...
	if (videoIndex >= 0) {
		...
		if (str == NULL) {
			ALOGV("Doesn't support video type %x\n", st->codec->codec_id);
			goto exit;
		} else
			ALOGV(
					"Video: %s:width:%d, height:%d found.\n", str, st->codec->width, st->codec->height);
		...
		mTracks[videoIndex].mExtraData =
				mTracks[videoIndex].mFormat->mcodecMeta;
		mTracks[videoIndex].mExtraSize =
				mTracks[videoIndex].mFormat->mCodecMetaSize;
	}
	...
	if (audioIndex >= 0) {
		...
		str = convertCodecIdToMimeType(st->codec->codec_id);
		if (str == NULL) {
			ALOGV("Doesn't support audio type %x\n", st->codec->codec_id);
			goto exit;
		} else
			ALOGV(
					"Audio: %s, rate:%d, channels:%d found.\n", str, st->codec->sample_rate, st->codec->channels);
		...
		mTracks[audioIndex].mExtraData =
				mTracks[audioIndex].mFormat->mcodecMeta;
		mTracks[audioIndex].mExtraSize =
				mTracks[audioIndex].mFormat->mCodecMetaSize;
	...
	if (subtitleIndex >= 0) {
		...
		str = convertCodecIdToMimeType(st->codec->codec_id);
		if (str == NULL) {
			ALOGV("Doesn't support subtitle type %x\n", st->codec->codec_id);
			goto exit;
		} else
			ALOGV("Subtitle: %s found.\n", str);
		...
		mTracks[subtitleIndex].mExtraData =
				mTracks[subtitleIndex].mFormat->mcodecMeta;
		mTracks[subtitleIndex].mExtraSize =
				mTracks[subtitleIndex].mFormat->mCodecMetaSize;
	}
...
status_t FFDemux::seekTo(int64_t timeUs, int mode)
{
	...
	mPTSPopulator->reset();
	{
		...
		if (av_seek_frame(mFormatContext, -1 /* default stream */, seekPosition, seekFlag) < 0)
		{
...
{
	return timeInStreamTime * AV_TIME_BASE * st->time_base.num
			/ st->time_base.den;
}
...
status_t FFDemux::advance()
{
	...
	do
	{
		...
		{
			currentTrack = -1;
			ALOGE("Opps...[ERROR]av_read_frame:%s.\n", av_err2str(mStatus));
			delete packet;
			return mStatus;
		}
		if (packet->size == 0)
		...
		{
			ALOGW("got one video packet, videoIndex=%d", videoIndex);
			...
			if (packet->stream_index == audioStreamIndex)
			{
				...
				{
					ALOGW("got one subtitle packet");
					av_dup_packet(packet);
					mTracks[subtitleIndex].mPacketQueue.push_back(packet);
				}
				else
				{
					ALOGW("what the hell is this");
					av_free_packet(packet);
					continue;
				}
		break;
...
int FFDemux::select_current_track() {
	...
	if (videoStreamIndex >= 0 && mTracks[videoIndex].mPacketQueue.size() > 0)
		return videoIndex;
			...
			&& mTracks[audioIndex].mPacketQueue.size() > 0)
		return audioIndex;
	else if (subtitleStreamIndex >= 0
			&& mTracks[subtitleIndex].mPacketQueue.size() > 0)
		return subtitleIndex;
	return -1;
}
...
status_t FFDemux::readSampleData(char* buf, size_t& len, int offset)
{
	...
	ALOGD( "%s;%s;%d, currentTrack =%d\n", __FILE__, __FUNCTION__, __LINE__, currentTrack);
	...
	if (mTracks[currentTrack].mExtraPushed)
	{
		...
		formatter = mTracks[currentTrack].mFormat;
		...
		{
			ALOGD("packet.size=%d", packet->size);
		}

		newPacketLen = formatter->getPackageSize(packet->data, packet->size);
		destLen = (len - offset) > newPacketLen ? newPacketLen : (len - offset);

		formatter->formatPackage(
														(uint8_t*) packet->data,
														packet->size,
														(uint8_t*) (buf + offset),
														destLen);
				   ...
				   (packet->stream_index == subtitleStreamIndex))
		{
			...
			const int64_t ptsFromFFmpeg =
																	   (packet->pts == static_cast<int64_t>(AV_NOPTS_VALUE)) ?
																		kUnknownPTS :
																		convertStreamTimeToUs(st, packet->pts);

			const int64_t dtsFromFFmpeg =
																	   (packet->dts == static_cast<int64_t>(AV_NOPTS_VALUE)) ?
																		kUnknownPTS :
																		convertStreamTimeToUs(st, packet->dts);

			const int64_t predictedPTSInUs = mPTSPopulator->computePTS(
																																packet->stream_index, ptsFromFFmpeg, dtsFromFFmpeg,
																																isKeyFrame);
			const int64_t normalizedPTSInUs =
					        (predictedPTSInUs == kUnknownPTS) ?
							dtsFromFFmpeg - mStartTimeUs :
							       ((predictedPTSInUs - mStartTimeUs < 0 && predictedPTSInUs - mStartTimeUs > -10) ?
									0 :
									predictedPTSInUs - mStartTimeUs);
			// starttime may exceed pts a little in some ugly streams.
	...
	else
	{
		...
		if (extraLen > 0) {
			int destLen = (len - offset) > extraLen ? extraLen : (len - offset);
