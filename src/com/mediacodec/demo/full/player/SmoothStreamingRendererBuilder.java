 ...
 */
package com.mediacodec.demo.full.player;
...
import com.mediacodec.demo.full.player.DemoPlayer.RendererBuilder;
import com.mediacodec.demo.full.player.DemoPlayer.RendererBuilderCallback;
 ...
 */
public class SmoothStreamingRendererBuilder implements RendererBuilder, ManifestCallback<SmoothStreamingManifest>
{
    ...
    public SmoothStreamingRendererBuilder(String userAgent, String url, String contentId, MediaDrmCallback drmCallback, TextView debugTextView)
    {
        ...
        for (int i = 0; i < manifest.streamElements.length; i++)
        {
            if (manifest.streamElements[i].type == StreamElement.TYPE_AUDIO)
            {
                ...
                if (manifest.streamElements[i].type == StreamElement.TYPE_TEXT)
                {
                    ...
                    if (videoStreamElementIndex == -1 && manifest.streamElements[i].type == StreamElement.TYPE_VIDEO)
                    {
                        ...
                        for (int j = 0; j < streamElement.tracks.length; j++)
                        {
                            ...
                            {
                                // The device isn't capable of playing this
                                // stream.
                            }
                        }
                    }
        ...
        else
        {
            ...
            for (int i = 0; i < manifest.streamElements.length; i++)
            {
                if (manifest.streamElements[i].type == StreamElement.TYPE_AUDIO)
                {
                    ...
                    audioChunkSources[audioStreamElementCount] = new SmoothStreamingChunkSource(url, manifest, i, new int[]
                    { 0 }, audioDataSource, audioFormatEvaluator);
                    audioStreamElementCount++;
                }
            }
        ...
        else
        {
            ...
            for (int i = 0; i < manifest.streamElements.length; i++)
            {
                if (manifest.streamElements[i].type == StreamElement.TYPE_TEXT)
                {
                    ...
                    textChunkSources[textStreamElementCount] = new SmoothStreamingChunkSource(url, manifest, i, new int[]
                    { 0 }, ttmlDataSource, ttmlFormatEvaluator);
                    textStreamElementCount++;
                }
            }
    ...
    private static class V18Compat
    {
        ...
        public static DrmSessionManager getDrmSessionManager(UUID uuid, DemoPlayer player, MediaDrmCallback drmCallback) throws UnsupportedSchemeException
        {
            return new StreamingDrmSessionManager(uuid, player.getPlaybackLooper(), drmCallback, player.getMainHandler(), player);
        }
