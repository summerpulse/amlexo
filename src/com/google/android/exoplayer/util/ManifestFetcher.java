 ...
 *
 * @param <T>
 *            The type of the manifest being parsed.
 */
public abstract class ManifestFetcher<T> extends AsyncTask<String, Void, T>
{
    ...
    /**
     * Reads the {@link InputStream} and parses it into a manifest. Invoked from
     * the {@link AsyncTask}'s background thread.
     *
     * @param stream
     ...
     *            The content id of the media.
     * @param baseUrl
     *            Required where the manifest contains urls that are relative to
     *            a base url. May be null where this is not the case.
     * @throws IOException
     *             If an error occurred loading the data.
     * @throws ParserException
     *             If an error occurred parsing the loaded data.
     */
    protected abstract T parse(InputStream stream, String inputEncoding, String contentId, Uri baseUrl) throws IOException, ParserException;
