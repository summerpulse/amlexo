 ...
 *
 * @see <a
 *      href="http://msdn.microsoft.com/en-us/library/ee673436(v=vs.90).aspx">
...
public class SmoothStreamingManifestParser
{
     ...
     *             If a problem occurred parsing the xml as a smooth streaming
     *             manifest.
     */
    ...
    /**
     * A base class for parsers that parse components of a smooth streaming
     * manifest.
     */
    private static abstract class ElementParser
    {
        ...
        /**
         * Stash an attribute that may be normalized at this level. In other
         * words, an attribute that may have been pulled up from the child
         * elements because its value was the same in all children.
         * <p>
         * Stashing an attribute allows child element parsers to retrieve the
         * values of normalized attributes using
         * {@link #getNormalizedAttribute(String)}.
         *
         * @param key
        ...
        /**
         * Attempt to retrieve a stashed normalized attribute. If there is no
         * stashed attribute with the provided name, the parent element parser
         * will be queried, and so on up the chain.
         *
         * @param key
         *            The name of the attribute.
        ...
        /**
         * @param xmlParser
         *            The underlying {@link XmlPullParser}
         * @throws ParserException
         */
    ...
    private static class SmoothStreamMediaParser extends ElementParser
    {
        ...
        public Object build()
        {
            ...
            return new SmoothStreamingManifest(majorVersion, minorVersion, timeScale, duration, lookAheadCount, protectionElement, streamElementArray);
        }
    ...
    private static class StreamElementParser extends ElementParser
    {
        ...
        private void parseStreamFragmentStartTag(XmlPullParser parser) throws ParserException
        {
            ...
            if (startTimes[chunkIndex] == -1L)
            {
                if (chunkIndex == 0)
                {
                    ...
                    if (previousChunkDuration != -1L)
                    {
                        // Infer the start time from the previous chunk's start
                        // time and duration.
                        startTimes[chunkIndex] = startTimes[chunkIndex - 1] + previousChunkDuration;
                    }
                    else
                    {
                        // We don't have the start time, and we're unable to
                        // infer it.
        ...
        public Object build()
        {
            ...
            return new StreamElement(type, subType, timeScale, name, qualityLevels, url, maxWidth, maxHeight, displayWidth, displayHeight, language, trackElements, startTimes);
        }
    ...
    private static class TrackElementParser extends ElementParser
    {
        ...
        public Object build()
        {
            ...
            return new TrackElement(index, bitrate, fourCC, csdArray, profile, level, maxWidth, maxHeight, samplingRate, channels, packetSize, audioTag, bitPerSample, nalUnitLengthField, content);
        }
