...
public class SmoothStreamingChunkSource implements ChunkSource
{
    ...
    /**
     * @param baseUrl
     *            The base URL for the streams.
     * @param manifest
     *            The manifest parsed from {@code baseUrl + "/Manifest"}.
     * @param streamElementIndex
     *            The index of the stream element in the manifest to be provided
     *            by the source.
     * @param trackIndices
     *            The indices of the tracks within the stream element to be
     *            considered by the source. May be null if all tracks within the
     *            element should be considered.
     * @param dataSource
     *            A {@link DataSource} suitable for loading the media data.
     * @param formatEvaluator
     *            Selects from the available formats.
     */
    public SmoothStreamingChunkSource(String baseUrl, SmoothStreamingManifest manifest, int streamElementIndex, int[] trackIndices, DataSource dataSource, FormatEvaluator formatEvaluator)
    {
        ...
        for (int i = 0; i < trackCount; i++)
        {
            ...
            formats[i] = new SmoothStreamingFormat(String.valueOf(trackIndex), trackElement.mimeType, trackElement.maxWidth, trackElement.maxHeight, trackElement.numChannels, trackElement.sampleRate, trackElement.bitrate, trackIndex);
            ...
            int trackType = streamElement.type == StreamElement.TYPE_VIDEO ? Track.TYPE_VIDEO : Track.TYPE_AUDIO;
            FragmentedMp4Extractor extractor = new FragmentedMp4Extractor(FragmentedMp4Extractor.WORKAROUND_EVERY_VIDEO_FRAME_IS_SYNC_FRAME);
    ...
    public final void getChunkOperation(List<? extends MediaChunk> queue, long seekPositionUs, long playbackPositionUs, ChunkOperationHolder out)
    {
        ...
        if (selectedFormat == null)
        {
            ...
            if (out.queueSize == queue.size() && out.chunk != null && out.chunk.format.id.equals(evaluation.format.id))
            {
                // We already have a chunk, and the evaluation hasn't changed
                // either the format or the size
        ...
        String requestUrl = streamElement.buildRequestUrl(selectedFormat.trackIndex, nextChunkIndex);
        Uri uri = Uri.parse(baseUrl + '/' + requestUrl);
        Chunk mediaChunk = newMediaChunk(selectedFormat, uri, null, extractors.get(Integer.parseInt(selectedFormat.id)), dataSource, nextChunkIndex, isLastChunk, streamElement.getStartTimeUs(nextChunkIndex),
    ...
    private static MediaFormat getMediaFormat(StreamElement streamElement, int trackIndex)
    {
        ...
        if (streamElement.type == StreamElement.TYPE_VIDEO)
        {
            ...
            if (streamElement.type == StreamElement.TYPE_AUDIO)
            {
                List<byte[]> csd;
    ...
    private static MediaChunk newMediaChunk(Format formatInfo, Uri uri, String cacheKey, FragmentedMp4Extractor extractor, DataSource dataSource, int chunkIndex, boolean isLast, long chunkStartTimeUs, long nextChunkStartTimeUs, int trigger)
    {
        ...
        // In SmoothStreaming each chunk contains sample timestamps relative to
        // the start of the chunk.
        // To convert them the absolute timestamps, we need to set
        // sampleOffsetUs to -chunkStartTimeUs.
