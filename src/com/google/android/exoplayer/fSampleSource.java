 ...
 */
// TODO: This implementation needs to be fixed so that its methods are
// non-blocking (either
// through use of a background thread, or through changes to the framework's
// MediaExtractor API).
@TargetApi(16)
public final class fSampleSource implements SampleSource
{
    ...
    public fSampleSource(Context context, Uri uri, Map<String, String> headers, int downstreamRendererCount)
    {
    ...
    public boolean prepare() throws IOException
    {
        if (!prepared)
        {
            ...
            for (int i = 0; i < trackStates.length; i++)
            {
                ...
                long duration = format.containsKey(android.media.MediaFormat.KEY_DURATION) ? format.getLong(android.media.MediaFormat.KEY_DURATION) : TrackRenderer.UNKNOWN_TIME_US;
    ...
    {
        // MediaExtractor takes care of buffering and blocks until it has
        // samples, so we can always
        // return true here. Although note that the blocking behavior is itself
        // as bug, as per the
        // TODO further up this file. This method will need to return something
        // else as part of fixing
        // the TODO.
        return true;
    }
    ...
    public int readData(int track, long playbackPositionUs, FormatHolder formatHolder, SampleHolder sampleHolder, boolean onlyReadDiscontinuity)
    {
        ...
        {
            formatHolder.format = MediaFormat.createFromFrameworkMediaFormatV16(extractor.getTrackFormat(track));
    ...
    public void seekToUs(long timeUs)
    {
        ...
        {
            // Avoid duplicate calls to the underlying extractor's seek method
