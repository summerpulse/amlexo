...
/**
 * A {@link TrackRenderer} for textual subtitles. The actual rendering of each
 * line of text to a suitable output (e.g. the display) is delegated to a
 * {@link TextRenderer}.
 */
@TargetApi(16)
public class TextTrackRenderer extends TrackRenderer implements Callback
{
    ...
    public interface TextRenderer
    {
         ...
         *
         * @param text
         *            The text to render, or null if no text is to be rendered.
         */
    ...
    /**
     * @param source
     *            A source from which samples containing subtitle data can be
     *            read.
     * @param subtitleParser
     *            A subtitle parser that will parse Subtitle objects from the
     *            source.
     * @param textRenderer
     *            The text renderer.
     * @param textRendererLooper
     *            The looper associated with the thread on which textRenderer
     *            should be invoked. If the renderer makes use of standard
     *            Android UI components, then this should normally be the looper
     *            associated with the applications' main thread, which can be
     *            obtained using {@link android.app.Activity#getMainLooper()}.
     *            Null may be passed if the renderer should be invoked directly
     *            on the player's internal rendering thread.
     */
    ...
    private void seekToInternal(long timeUs)
    {
        ...
        source.seekToUs(timeUs);
        if (subtitle != null && (timeUs < subtitle.getStartTime() || subtitle.getLastEventTime() <= timeUs))
        {
            subtitle = null;
        }
        resetSampleData();
    ...
    protected void doSomeWork(long timeUs) throws ExoPlaybackException
    {
        ...
        // We're iterating through the events in a subtitle. Set
        // textRendererNeedsUpdate if we advance
        // to the next event.
        if (subtitle != null)
        {
        ...
        // We don't have a subtitle. Try and read the next one from the source,
        // and if we succeed then
        ...
        if (subtitle == null)
        {
            ...
            try
            {
                ...
                if (result == SampleSource.SAMPLE_READ)
                {
                    ...
                    InputStream subtitleInputStream = new ByteArrayInputStream(sampleHolder.data.array(), 0, sampleHolder.size);
        ...
        // Update the text renderer if we're both playing and
        // textRendererNeedsUpdate is set.
    ...
    {
        // Don't block playback whilst subtitles are loading.
        // Note: To change this behavior, it will be necessary to consider
        // [redacted].
        return true;
    }
    ...
    {
        return ((nextSubtitleEventIndex == -1) || (nextSubtitleEventIndex >= subtitle.getEventTimeCount())) ? Long.MAX_VALUE : (subtitle.getEventTime(nextSubtitleEventIndex));
    }
