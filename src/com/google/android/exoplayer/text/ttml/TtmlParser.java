...
public class TtmlParser implements SubtitleParser
{
    ...
    private TtmlNode parseNode(XmlPullParser parser, TtmlNode parent)
    {
        ...
        {
            // TODO: check if it's safe to ignore the namespace of attributes as
            // follows.
            String attr = parser.getAttributeName(i).replaceFirst("^.*:", "");
            String value = parser.getAttributeValue(i);
            if (attr.equals(ATTR_BEGIN))
            {
                startTime = parseTimeExpression(value, DEFAULT_FRAMERATE, DEFAULT_SUBFRAMERATE, DEFAULT_TICKRATE);
            }
            else
                if (attr.equals(ATTR_END))
                {
                    endTime = parseTimeExpression(value, DEFAULT_FRAMERATE, DEFAULT_SUBFRAMERATE, DEFAULT_TICKRATE);
                }
                else
        ...
        if (endTime == TtmlNode.UNDEFINED_TIME)
        {
            if (duration > 0)
            {
                ...
                if (parent != null && parent.endTimeUs != TtmlNode.UNDEFINED_TIME)
                {
                    // If the end time remains unspecified, then it should be
    ...
    {
        if (tag.equals(TtmlNode.TAG_TT) || tag.equals(TtmlNode.TAG_HEAD) || tag.equals(TtmlNode.TAG_BODY) || tag.equals(TtmlNode.TAG_DIV) || tag.equals(TtmlNode.TAG_P) || tag.equals(TtmlNode.TAG_SPAN) || tag.equals(TtmlNode.TAG_BR)
                || tag.equals(TtmlNode.TAG_STYLE) || tag.equals(TtmlNode.TAG_STYLING) || tag.equals(TtmlNode.TAG_LAYOUT) || tag.equals(TtmlNode.TAG_REGION) || tag.equals(TtmlNode.TAG_METADATA) || tag.equals(TtmlNode.TAG_SMPTE_IMAGE)
                || tag.equals(TtmlNode.TAG_SMPTE_DATA) || tag.equals(TtmlNode.TAG_SMPTE_INFORMATION))
        {
            return true;
        }
        return false;
     ...
     * <p>
     * For the format of a time expression, see: <a
     * href="http://www.w3.org/TR/ttaf1-dfxp/#timing-value-timeExpression"
     * >timeExpression</a>
     *
     * @param time
     *            A string that includes the time expression.
     * @param frameRate
     ...
     *             If the given string does not contain a valid time expression.
     */
    private static long parseTimeExpression(String time, int frameRate, int subframeRate, int tickRate)
    {
