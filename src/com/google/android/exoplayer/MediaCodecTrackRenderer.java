...
public abstract class MediaCodecTrackRenderer extends TrackRenderer
{
     ...
     * {@link MediaCodecTrackRenderer} events.
     */
    ...
    /**
     * If the {@link MediaCodec} is hotswapped (i.e. replaced during playback),
     * this is the period of time during which {@link #isReady()} will report
     * true regardless of whether the new codec has output frames that are ready
     * to be rendered.
     * <p>
     * This allows codec hotswapping to be performed seamlessly, without
     * interrupting the playback of other renderers, provided the new codec is
     * able to decode some frames within this time period.
     */
    ...
    /**
     * Codec configuration data has been written into the next buffer, but that
     * buffer still needs to be returned to the codec.
     */
     ...
     *            For use with encrypted media. May be null if support for
     *            encrypted media is not required.
    ...
    protected int doPrepare() throws ExoPlaybackException
    {
        ...
        {
            // TODO: Right now this is getting the mime types of the container
            // format
            // (e.g. audio/mp4 and video/mp4 for fragmented mp4). It needs to be
     ...
     * override this method if they wish to configure the codec with a non-null
     * surface.
     **/
    ...
    protected final void maybeInitCodec() throws ExoPlaybackException
    {
        ...
        if (drmInitData != null)
        {
            ...
            if (drmSessionState == DrmSessionManager.STATE_ERROR)
            {
                ...
                if (drmSessionState == DrmSessionManager.STATE_OPENED || drmSessionState == DrmSessionManager.STATE_OPENED_WITH_KEYS)
                {
        ...
        try
        {
            ...
            Log.d("MediaCodecTrackRenderer", "mimeType="+mimeType);
        ...
        {
            DecoderInitializationException exception = new DecoderInitializationException(selectedDecoderName, format, e);
    ...
    protected long getBufferedPositionUs()
    {
        ...
        return sourceBufferedPosition == UNKNOWN_TIME_US || sourceBufferedPosition == END_OF_TRACK_US ? sourceBufferedPosition : Math.max(sourceBufferedPosition, getCurrentPositionUs());
    }
    ...
    protected void doSomeWork(long timeUs) throws ExoPlaybackException
    {
        ...
        try
        {
            ...
            if (format == null)
            {
                ...
                if (codec == null && !shouldInitCodec() && getState() == TrackRenderer.STATE_STARTED)
                {
    ...
    private void flushCodec() throws ExoPlaybackException
    {
        ...
        {
            // Any reconfiguration data that we send shortly before the flush
            // may be discarded. We
            // avoid this issue by sending reconfiguration data following every
            // flush.
    ...
    /**
     * @return True if it may be possible to feed more input data. False
     *         otherwise.
     * @throws IOException
     *             If an error occurs reading data from the upstream source.
     * @throws ExoPlaybackException
     *             If an error occurs feeding the input buffer.
     */
    private boolean feedInputBuffer() throws IOException, ExoPlaybackException
    {
        ...
        if (waitingForKeys)
        {
            // We've already read an encrypted sample into sampleHolder, and are
            // waiting for keys.
            result = SampleSource.SAMPLE_READ;
        }
        else
        {
            // For adaptive reconfiguration OMX decoders expect all
            // reconfiguration data to be supplied
            // at the start of the buffer that also contains the first frame in
            // the new format.
        ...
        if (result == SampleSource.FORMAT_READ)
        {
            ...
            {
                // We received two formats in a row. Clear the current buffer of
                // any reconfiguration data
        ...
        if (result == SampleSource.END_OF_STREAM)
        {
            if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING)
            {
                // We received a new format immediately before the end of the
                // stream. We need to clear
                // the corresponding reconfiguration data from the current
                // buffer, but re-write it into
                // a subsequent buffer if there are any (e.g. if the user seeks
                // backwards).
                sampleHolder.data.clear();
                codecReconfigurationState = RECONFIGURATION_STATE_WRITE_PENDING;
            }
        ...
        {
            // TODO: Find out if it's possible to supply samples prior to the
            // first sync
            // frame for HE-AAC.
            if ((sampleHolder.flags & MediaExtractor.SAMPLE_FLAG_SYNC) == 0)
            {
                ...
                if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING)
                {
                    // The buffer we just cleared contained reconfiguration
                    // data. We need to re-write this
        ...
        try
        {
            ...
            if (sampleEncrypted)
            {
                MediaCodec.CryptoInfo cryptoInfo = getFrameworkCryptoInfo(sampleHolder, adaptiveReconfigurationBytes);
    ...
    private static MediaCodec.CryptoInfo getFrameworkCryptoInfo(SampleHolder sampleHolder, int adaptiveReconfigurationBytes)
    {
        ...
        // There must be at least one sub-sample, although numBytesOfClearData
        // is permitted to be
        // null if it contains no clear data. Instantiate it if needed, and add
        // the reconfiguration
    ...
    private boolean shouldWaitForKeys(boolean sampleEncrypted) throws ExoPlaybackException
    {
        ...
        if (drmManagerState != DrmSessionManager.STATE_OPENED_WITH_KEYS && (sampleEncrypted || !playClearSamplesWithoutKeys))
        {
            return true;
        }
        return false;
     ...
     *
     * @param formatHolder
     *            Holds the new format.
     * @throws ExoPlaybackException
     *             If an error occurs reinitializing the {@link MediaCodec}.
     */
    ...
    /**
     * Determines whether the existing {@link MediaCodec} should be reconfigured
     * for a new format by sending codec specific initialization data at the
     * start of the next input buffer. If true is returned then the
     * {@link MediaCodec} instance will be reconfigured in this way. If false is
     * returned then the instance will be released, and a new instance will be
     * created for the new format.
     * <p>
     * The default implementation returns false.
     *
     * @param codec
     *            The existing {@link MediaCodec} instance.
     * @param codecIsAdaptive
     *            Whether the codec is adaptive.
     * @param oldFormat
     *            The format for which the existing instance is configured.
     * @param newFormat
     *            The new format.
     * @return True if the existing instance can be reconfigured. False
     *         otherwise.
     */
    ...
    {
        return format != null && !waitingForKeys && (sourceIsReady || outputIndex >= 0 || isWithinHotswapPeriod());
    }
    ...
    /**
     * @return True if it may be possible to drain more output data. False
     *         otherwise.
     * @throws ExoPlaybackException
     *             If an error occurs draining the output buffer.
     */
    private boolean drainOutputBuffer(long timeUs) throws ExoPlaybackException
    {
        ...
        boolean decodeOnly = decodeOnlyPresentationTimestamps.contains(outputBufferInfo.presentationTimeUs);
        if (processOutputBuffer(timeUs, codec, outputBuffers[outputIndex], outputBufferInfo, outputIndex, decodeOnly))
        {
            if (decodeOnly)
            {
     ...
     *
     * @return True if the output buffer was processed (e.g. rendered or
     *         discarded) and hence is no longer required. False otherwise.
     * @throws ExoPlaybackException
     *             If an error occurs processing the output buffer.
     */
    protected abstract boolean processOutputBuffer(long timeUs, MediaCodec codec, ByteBuffer buffer, MediaCodec.BufferInfo bufferInfo, int bufferIndex, boolean shouldSkip) throws ExoPlaybackException;
