...
public class FFExtractor
{
    ...
    /* Native methods. */
    /**
     * Sets the DataSource object to be used as the data source for this
     * extractor {@hide}
     */
    // private native final void setDataSource_native(DataSource source) throws
    // IOException;

    private native final void setDataSource_native(String path, String[] keys, String[] values) throws IOException;
     ...
     * @param offset
     *            the offset into the file where the data to be extracted
     *            starts, in bytes
     * @param length
     *            the length in bytes of the data to be extracted
     */
    private native final void setDataSource_native(FileDescriptor fd, long offset, long length) throws IOException;
    ...
    /**
     * Subsequent calls to {@link #readSampleData}, {@link #getSampleTrackIndex}
     * and {@link #getSampleTime} only retrieve information for the subset of
     * tracks selected. Selecting the same track multiple times has no effect,
     * the track is only selected once.
     */
     ...
     * starting at the given offset. Returns the sample size (or -1 if no more
    ...
    /**
     * Returns the current sample's presentation time in microseconds. or -1 if
     ...
     * fetching of data). This API only returns a meaningful result if
     * {@link #getCachedDuration} indicates the presence of a cache, i.e. does
     * NOT return -1.
     */
