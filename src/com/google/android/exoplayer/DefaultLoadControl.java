...
/**
 * A {@link LoadControl} implementation that allows loads to continue in a
 * sequence that prevents any loader from getting too far ahead or behind any of
 * the other loaders.
 * <p>
 * Loads are scheduled so as to fill the available buffer space as rapidly as
 * possible. Once the duration of buffered media and the buffer utilization both
 * exceed respective thresholds, the control switches to a draining state during
 * which no loads are permitted to start. During draining periods, resources
 * such as the device radio have an opportunity to switch into low power modes.
 * The control reverts back to the loading state when either the duration of
 * buffered media or the buffer utilization fall below respective thresholds.
 * <p>
 * This implementation of {@link LoadControl} integrates with
 * {@link NetworkLock}, by registering itself as a task with priority
 * {@link NetworkLock#STREAMING_PRIORITY} during loading periods, and
 * unregistering itself during draining periods.
 */
public class DefaultLoadControl implements LoadControl
{
     ...
     * {@link DefaultLoadControl} events.
     */
    public interface EventListener
    {

        /**
         * Invoked when the control transitions from a loading to a draining
         * state, or vice versa.
         *
         * @param loading
         *            Whether the control is now in a loading state.
         */
    ...
    /**
     * Constructs a new instance, using the {@code DEFAULT_*} constants defined
     * in this class.
     *
     * @param allocator
     *            The {@link Allocator} used by the loader.
     */
    ...
    {
        this(allocator, eventHandler, eventListener, DEFAULT_LOW_WATERMARK_MS, DEFAULT_HIGH_WATERMARK_MS, DEFAULT_LOW_POOL_LOAD, DEFAULT_HIGH_POOL_LOAD);
    }
     ...
     *            A listener of events. May be null if delivery of events is not
     *            required.
     * @param lowWatermarkMs
     *            The minimum duration of media that can be buffered for the
     *            control to be in the draining state. If less media is
     *            buffered, then the control will transition to the filling
     *            state.
     * @param highWatermarkMs
     *            The minimum duration of media that can be buffered for the
     *            control to transition from filling to draining.
     * @param lowPoolLoad
     *            The minimum fraction of the buffer that must be utilized for
     *            the control to be in the draining state. If the utilization is
     *            lower, then the control will transition to the filling state.
     * @param highPoolLoad
     *            The minimum fraction of the buffer that must be utilized for
     *            the control to transition from the loading state to the
     *            draining state.
     */
    public DefaultLoadControl(Allocator allocator, Handler eventHandler, EventListener eventListener, int lowWatermarkMs, int highWatermarkMs, float lowPoolLoad, float highPoolLoad)
    {
    ...
    public boolean update(Object loader, long playbackPositionUs, long nextLoadPositionUs, boolean loading, boolean failed)
    {
        ...
        boolean loaderStateChanged = loaderState.bufferState != loaderBufferState || loaderState.nextLoadPositionUs != nextLoadPositionUs || loaderState.loading != loading || loaderState.failed != failed;
        ...
        // If either of the individual states have changed, update the shared
        // control state.
        ...
        return allocatedSize < targetBufferSize && nextLoadPositionUs != -1 && nextLoadPositionUs <= maxLoadStartPositionUs;
    }
    ...
    private int getLoaderBufferState(long playbackPositionUs, long nextLoadPositionUs)
    {
        ...
        else
        {
            ...
            return timeUntilNextLoadPosition > highWatermarkUs ? ABOVE_HIGH_WATERMARK : timeUntilNextLoadPosition < lowWatermarkUs ? BELOW_LOW_WATERMARK : BETWEEN_WATERMARKS;
        }
    }
    ...
    private int getBufferPoolState(int allocatedSize)
    {
        ...
        return bufferPoolLoad > highPoolLoad ? ABOVE_HIGH_WATERMARK : bufferPoolLoad < lowPoolLoad ? BELOW_LOW_WATERMARK : BETWEEN_WATERMARKS;
    }
    ...
    private void updateControlState()
    {
        ...
        fillingBuffers = !loaders.isEmpty() && !finished && !failed && (highestState == BELOW_LOW_WATERMARK || (highestState == BETWEEN_WATERMARKS && fillingBuffers));
        if (fillingBuffers && !streamingPrioritySet)
        {
            ...
            if (!fillingBuffers && streamingPrioritySet && !loading)
            {
        ...
        if (fillingBuffers)
        {
            for (int i = 0; i < loaders.size(); i++)
            {
                ...
                if (loaderTime != -1 && (maxLoadStartPositionUs == -1 || loaderTime < maxLoadStartPositionUs))
                {
