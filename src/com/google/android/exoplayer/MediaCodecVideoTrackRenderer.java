...
public class MediaCodecVideoTrackRenderer extends MediaCodecTrackRenderer
{
     ...
     * {@link MediaCodecVideoTrackRenderer} events.
     */
    public interface EventListener extends MediaCodecTrackRenderer.EventListener
    {
         ...
         * Dropped frames are reported whenever the renderer is stopped having
         * dropped frames, and optionally, whenever the count reaches a
         * specified threshold whilst the renderer is started.
         *
         * @param count
         *            The number of dropped frames.
         * @param elapsed
         *            The duration in milliseconds over which the frames were
         *            dropped. This duration is timed from when the renderer was
         *            started or from when dropped frames were last reported
         *            (whichever was more recent), and not from when the first
         *            of the reported drops occurred.
         */
        void onDroppedFrames(int count, long elapsed);

        /**
         * Invoked each time there's a change in the size of the video being
         * rendered.
         *
         * @param width
        ...
        /**
         * Invoked when a frame is rendered to a surface for the first time
         * following that surface having been set as the target for the
         * renderer.
         *
         * @param surface
     ...
     * The message object should be the target {@link Surface}, or null.
     */
     ...
     *            The scaling mode to pass to
     *            {@link MediaCodec#setVideoScalingMode(int)}.
     * @param allowedJoiningTimeMs
     *            The maximum duration in milliseconds for which this video
     ...
     *            required.
     * @param maxDroppedFrameCountToNotify
     *            The maximum number of frames that can be dropped between
     *            invocations of
     *            {@link EventListener#onDroppedFrames(int, long)}.
     */
     ...
     *            {@link EventListener#onDroppedFrames(int, long)}.
     */
    public MediaCodecVideoTrackRenderer(SampleSource source, DrmSessionManager drmSessionManager, boolean playClearSamplesWithoutKeys, int videoScalingMode, long allowedJoiningTimeMs, Handler eventHandler, EventListener eventListener,
    ...
    protected boolean isReady()
    {
        if (super.isReady())
        {
            // Ready. If we were joining then we've now joined, so clear the
            // joining deadline.
            joiningDeadlineUs = -1;
            return true;
        }
        else
            if (joiningDeadlineUs == -1)
            {
                // Not joining.
                return false;
            }
            else
                if (SystemClock.elapsedRealtime() * 1000 < joiningDeadlineUs)
                {
                ...
                {
                    // The joining deadline has been exceeded. Give up and clear
                    // the deadline.
                    joiningDeadlineUs = -1;
                    return false;
                }
    ...
    protected void onOutputFormatChanged(android.media.MediaFormat format)
    {
        ...
        currentWidth = hasCrop ? format.getInteger(KEY_CROP_RIGHT) - format.getInteger(KEY_CROP_LEFT) + 1 : format.getInteger(android.media.MediaFormat.KEY_WIDTH);
        currentHeight = hasCrop ? format.getInteger(KEY_CROP_BOTTOM) - format.getInteger(KEY_CROP_TOP) + 1 : format.getInteger(android.media.MediaFormat.KEY_HEIGHT);
    }
    ...
    protected boolean canReconfigureCodec(MediaCodec codec, boolean codecIsAdaptive, MediaFormat oldFormat, MediaFormat newFormat)
    {
        ...
        return newFormat.mimeType.equals(MimeTypes.VIDEO_H264) && oldFormat.mimeType.equals(MimeTypes.VIDEO_H264) && codecIsAdaptive || (oldFormat.width == newFormat.width && oldFormat.height == newFormat.height);
    }
    ...
    protected boolean processOutputBuffer(long timeUs, MediaCodec codec, ByteBuffer buffer, MediaCodec.BufferInfo bufferInfo, int bufferIndex, boolean shouldSkip)
    {
        ...
        if (getState() == TrackRenderer.STATE_STARTED && earlyUs < 30000)
        {
            if (earlyUs > 11000)
            {
                // We're a little too early to render the frame. Sleep until the
                // frame can be rendered.
                // Note: The 11ms threshold was chosen fairly arbitrarily.
                try
                {
                    // Subtracting 10000 rather than 11000 ensures that the
                    // sleep time will be at least 1ms.
