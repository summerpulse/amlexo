...
/* package */final class ExoPlayerImplInternal implements Handler.Callback
{
    ...
    public ExoPlayerImplInternal(Handler eventHandler, boolean playWhenReady, boolean[] rendererEnabledFlags, int minBufferMs, int minRebufferMs)
    {
        ...
        internalPlaybackThread = new HandlerThread(getClass().getSimpleName() + ":Handler")
        {
            ...
            public void run()
            {
                // Note: The documentation for Process.THREAD_PRIORITY_AUDIO
                // that states "Applications can
    ...
    {
        return bufferedPositionUs == TrackRenderer.UNKNOWN_TIME_US ? ExoPlayer.UNKNOWN_TIME : (int) (bufferedPositionUs / 1000);
    }

    public int getDuration()
    {
        return durationUs == TrackRenderer.UNKNOWN_TIME_US ? ExoPlayer.UNKNOWN_TIME : (int) (durationUs / 1000);
    }
            ...
            if (rendererEnabledFlags[i] && renderer.getState() == TrackRenderer.STATE_PREPARED)
            {
                ...
                else
                {
                    ...
                    if (trackDurationUs == TrackRenderer.UNKNOWN_TIME_US)
                    {
                        ...
                        if (trackDurationUs == TrackRenderer.MATCH_LONGEST_US)
                        {
                            // Do nothing.
                        }
                        else
    ...
    private boolean rendererReadyOrEnded(TrackRenderer renderer)
    {
        ...
        return minBufferDurationUs <= 0 || rendererBufferedPositionUs == TrackRenderer.UNKNOWN_TIME_US || rendererBufferedPositionUs == TrackRenderer.END_OF_TRACK_US || rendererBufferedPositionUs >= positionUs + minBufferDurationUs
                || (rendererDurationUs != TrackRenderer.UNKNOWN_TIME_US && rendererDurationUs != TrackRenderer.MATCH_LONGEST_US && rendererBufferedPositionUs >= rendererDurationUs);
    }
    ...
    {
        positionUs = timeSourceTrackRenderer != null && enabledRenderers.contains(timeSourceTrackRenderer) ? timeSourceTrackRenderer.getCurrentPositionUs() : mediaClock.getTimeUs();
    }
    ...
    private void doSomeWork() throws ExoPlaybackException
    {
        ...
        for (int i = 0; i < enabledRenderers.size(); i++)
        {
            ...
            else
            {
                ...
                if (rendererBufferedPositionUs == TrackRenderer.UNKNOWN_TIME_US)
                {
                    ...
                    if (rendererBufferedPositionUs == TrackRenderer.END_OF_TRACK_US || (rendererDurationUs != TrackRenderer.UNKNOWN_TIME_US && rendererDurationUs != TrackRenderer.MATCH_LONGEST_US && rendererBufferedPositionUs >= rendererDurationUs))
                    {
        ...
        if (isEnded)
        {
            ...
            if (state == ExoPlayer.STATE_BUFFERING && allRenderersReadyOrEnded)
            {
                ...
                if (state == ExoPlayer.STATE_READY && !allRenderersReadyOrEnded)
                {
    ...
    private void resetInternal()
    {
        ...
        for (int i = 0; i < renderers.length; i++)
        {
            ...
            {
                // There's nothing we can do. Catch the exception here so that
                // other renderers still have
    ...
    private <T> void sendMessageInternal(int what, Object obj) throws ExoPlaybackException
    {
        ...
        if (state != ExoPlayer.STATE_IDLE && state != ExoPlayer.STATE_PREPARING)
        {
            // The message may have caused something to change that now requires
            // us to do work.
            handler.sendEmptyMessage(MSG_DO_SOME_WORK);
        }
    }
    ...
    private void setRendererEnabledInternal(int index, boolean enabled) throws ExoPlaybackException
    {
        ...
        if (rendererState != TrackRenderer.STATE_PREPARED && rendererState != TrackRenderer.STATE_ENABLED && rendererState != TrackRenderer.STATE_STARTED)
        {
            return;
        }
        ...
        else
        {
            ...
            {
                // We've been using timeSourceTrackRenderer to advance the
                // current position, but it's
                // being disabled. Sync mediaClock so that it can take over
