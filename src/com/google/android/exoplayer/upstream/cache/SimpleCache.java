...
public class SimpleCache implements Cache
{
    ...
    /**
     * Constructs the cache. The cache will delete any unrecognized files from
     * the directory. Hence the directory cannot be used to store other files.
     *
     * @param cacheDir
    ...
    public synchronized CacheSpan startReadWrite(String key, long position) throws InterruptedException
    {
        ...
        while (true)
        {
            ...
            {
                // Write case, lock not available. We'll be woken up when a
                // locked span is released (if the
                // released lock is for the requested key then we'll be able to
                // make progress) or when a
                // span is added to the cache (if the span is for the requested
                // key and covers the requested
                // position, then we'll become a read and be able to make
                // progress).
                wait();
            }
        }
    ...
    public synchronized File startFile(String key, long position, long length)
    {
        ...
        if (!cacheDir.exists())
        {
            // For some reason the cache directory doesn't exist. Make a best
            // effort to create it.
            removeStaleSpans();
            cacheDir.mkdirs();
        }
    ...
    public synchronized void commitFile(File file)
    {
        ...
        // If the file doesn't exist, don't add it to the in-memory
        // representation.
        if (!file.exists())
        {
            return;
        }
        // If the file has length 0, delete it and don't add it to the in-memory
        // representation.
    ...
    /**
     * Returns the cache {@link CacheSpan} corresponding to the provided lookup
     * {@link CacheSpan}.
     * <p>
     * If the lookup position is contained by an existing entry in the cache,
     * then the returned {@link CacheSpan} defines the file in which the data is
     * stored. If the lookup position is not contained by an existing entry,
     * then the returned {@link CacheSpan} defines the maximum extents of the
     * hole in the cache.
     *
     * @param lookupSpan
     *            A lookup {@link CacheSpan} specifying a key and position.
    ...
    private CacheSpan getSpan(CacheSpan lookupSpan)
    {
        ...
        if (floorSpan != null && floorSpan.position <= offset && offset < floorSpan.position + floorSpan.length)
        {
            ...
            {
                // The file has been deleted from under us. It's likely that
                // other files will have been
    ...
    /**
     * Scans all of the cached spans in the in-memory representation, removing
     * any for which files no longer exist.
     */
    ...
    public synchronized boolean isCached(String key, long position, long length)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            // We expect currentEndPosition to always equal (next.position +
            // next.length), but
            // perform a max check anyway to guard against the existence of
            // overlapping spans.
