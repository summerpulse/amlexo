...
/**
 * An {@link Allocator} that maintains a pool of fixed length byte arrays
 * (buffers).
 * <p>
 * An {@link Allocation} obtained from a {@link BufferPool} consists of the
 * whole number of these buffers. When an {@link Allocation} is released, the
...
public final class BufferPool implements Allocator
{
     ...
     * <p>
     * An existing array of byte arrays may be provided to form the start of the
     * allocation.
     *
     * @param size
     *            The total size required, in bytes.
     * @param existing
     *            Existing byte arrays to use as the start of the allocation.
     *            May be null.
    ...
    /* package */synchronized byte[][] allocate(int size, byte[][] existing)
    {
        ...
        {
            // Use a recycled buffer if one is available. Else instantiate a new
            // one.
            buffers[i] = recycledBufferCount > 0 ? recycledBuffers[--recycledBufferCount] : new byte[bufferLength];
        }
        return buffers;
    }
