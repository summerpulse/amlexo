...
public class HttpDataSource implements DataSource
{
    ...
    /**
     * Thrown when an error is encountered when trying to read from HTTP data
     * source.
     */
    ...
    /**
     * Thrown when an attempt to open a connection results in a response code
     ...
     *            An optional {@link Predicate}. If a content type is rejected
     *            by the predicate then a {@link InvalidContentTypeException} is
    ...
    {
        this(userAgent, contentTypePredicate, listener, DEFAULT_CONNECT_TIMEOUT_MILLIS, DEFAULT_READ_TIMEOUT_MILLIS);
    }
     ...
     *            The read timeout, in milliseconds. A timeout of zero is
     *            interpreted as an infinite timeout.
     */
    ...
    public long open(DataSpec dataSpec) throws HttpDataSourceException
    {
        ...
        if (dataSpec.length != C.LENGTH_UNBOUNDED && contentLength != C.LENGTH_UNBOUNDED && contentLength != dataSpec.length)
        {
            // The DataSpec specified a length and we resolved a length from the
            // response headers, but
    ...
    public int read(byte[] buffer, int offset, int readLength) throws HttpDataSourceException
    {
        ...
        if (read > 0)
        {
            ...
            if (dataLength != C.LENGTH_UNBOUNDED && dataLength != bytesRead)
            {
                // Check for cases where the server closed the connection having
                // not sent the correct amount
                // of data. We can only do this if we know the length of the
                // data we were expecting.
                throw new HttpDataSourceException(new UnexpectedLengthException(dataLength, bytesRead), dataSpec);
            }
    ...
    /**
     * Returns the current connection, or null if the source is not currently
     * opened.
     *
    ...
    /**
     * Returns the number of bytes that have been read since the most recent
     ...
     * {@link DataSpec}.
     * <p>
     * If the total length of the data being read is known, then this length
     * minus {@code bytesRead()} is returned. If the total length is unknown,
     * {@link C#LENGTH_UNBOUNDED} is returned.
     *
        ...
        if (!TextUtils.isEmpty(contentRangeHeader))
        {
            ...
            if (matcher.find())
            {
                try
                {
                    ...
                    if (contentLength < 0)
                    {
                        // Some proxy servers strip the Content-Length header.
                        // Fall back to the length
                        ...
                        if (contentLength != contentLengthFromRange)
                        {
                            ...
                            // assume the one with the larger value is correct.
                            // We have seen cases where carrier
                            // change one of them to reduce the size of a
                            // request, but it is unlikely anybody would
                            // increase it.
