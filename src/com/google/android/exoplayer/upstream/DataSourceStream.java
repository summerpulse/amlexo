...
/**
 * Loads data from a {@link DataSource} into an in-memory {@link Allocation}.
 * The loaded data can be consumed by treating the instance as a non-blocking
...
public final class DataSourceStream implements Loadable, NonBlockingInputStream
{
    ...
    /**
     * Thrown when an error is encountered trying to load data into a
     * {@link DataSourceStream}.
     */
    ...
    /**
     * Returns the length of the stream in bytes, or {@value C#LENGTH_UNBOUNDED}
     * if the length has yet to be determined.
     *
     * @return The length of the stream in bytes, or {@value C#LENGTH_UNBOUNDED}
     *         if the length has yet to be determined.
     */
    ...
    /**
     * Returns a byte array containing the loaded data. If the data is partially
     * loaded, this method returns the portion of the data that has been loaded
     * so far. If nothing has been loaded, null is returned. This method does
     * not use or update the current read position.
     * <p>
     * Note: The read methods provide a more efficient way of consuming the
     * loaded data. Use this method only when a freshly allocated byte[]
     * containing all of the loaded data is required.
     *
    ...
    /**
     * Reads data to either a target {@link ByteBuffer}, or to a target byte
     * array at a specified offset. The {@code readHead} is updated to reflect
     * the read that was performed.
     */
    ...
    public void load() throws IOException, InterruptedException
    {
        ...
        try
        {
            ...
            if (loadPosition == 0 && resolvedLength == C.LENGTH_UNBOUNDED)
            {
                ...
                {
                    throw new DataSourceStreamLoadException(new UnexpectedLengthException(dataSpec.length, resolvedLength));
                }
                this.resolvedLength = resolvedLength;
            }
            else
            {
                long remainingLength = resolvedLength != C.LENGTH_UNBOUNDED ? resolvedLength - loadPosition : C.LENGTH_UNBOUNDED;
                loadDataSpec = new DataSpec(dataSpec.uri, dataSpec.position + loadPosition, remainingLength, dataSpec.key);
                dataSource.open(loadDataSpec);
            }

            if (allocation == null)
            {
                int initialAllocationSize = resolvedLength != C.LENGTH_UNBOUNDED ? (int) resolvedLength : CHUNKED_ALLOCATION_INCREMENT;
            ...
            while (!loadCanceled && read > 0 && maybeMoreToLoad())
            {
                ...
                read = dataSource.read(buffers[writeFragmentIndex], writeFragmentOffset, writeFragmentRemainingLength);
                if (read > 0)
                {
                    loadPosition += read;
                    ...
                    if (resolvedLength == C.LENGTH_UNBOUNDED)
                    {
                        ...
                        if (resolvedLength != loadPosition)
                        {
                            throw new DataSourceStreamLoadException(new UnexpectedLengthException(resolvedLength, loadPosition));
                        }
            }
        }
        finally
