...
public final class FragmentedMp4Extractor
{
    ...
    /**
     * Flag to work around an issue in some video streams where every frame is
     * marked as a sync frame. The workaround overrides the sync frame flags in
     * the stream, forcing them to false except for the first sample in each
     * segment.
     * <p>
    ...
    /**
     * A moov atom was read. The parsed data can be read using
     * {@link #getFormat()} and {@link #getPsshInfo}.
     */
    public static final int RESULT_READ_INIT = 8;
    /**
     * A sidx atom was read. The parsed data can be read using
     * {@link #getIndex()}.
     */
    public static final int RESULT_READ_INDEX = 16;
    /**
     * The next thing to be read is a sample, but a {@link SampleHolder} was not
     * supplied.
     */
    public static final int RESULT_NEED_SAMPLE_HOLDER = 32;

    private static final int READ_TERMINATING_RESULTS = RESULT_NEED_MORE_DATA | RESULT_END_OF_STREAM | RESULT_READ_SAMPLE | RESULT_NEED_SAMPLE_HOLDER;
    private static final byte[] NAL_START_CODE = new byte[]
    { 0, 0, 0, 1 };
    private static final byte[] PIFF_SAMPLE_ENCRYPTION_BOX_EXTENDED_TYPE = new byte[]
    { -94, 57, 79, 82, 90, -101, 79, 20, -94, 68, 108, 66, 124, 100, -115, -12 };
    ...
    /**
     * @param workaroundFlags
     *            Flags to allow parsing of faulty streams.
     *            {@link #WORKAROUND_EVERY_VIDEO_FRAME_IS_SYNC_FRAME} is
     *            currently the only flag defined.
     */
     ...
     *
     * @return The pssh information. May be null if the MOOV atom has yet to be
     *         parsed of if it did not contain any pssh information.
     */
    ...
    /**
     * Sideloads pssh information into the extractor, so that it can be read
     * through {@link #getPsshInfo()}.
     *
     * @param uuid
     *            The UUID of the scheme for which information is being
     *            sideloaded.
     * @param data
    ...
    {
        // TODO: This is for SmoothStreaming. Consider using something other
        // than
        // FragmentedMp4Extractor.getPsshInfo to obtain the pssh data for that
        // use case, so that we can
        // remove this method.
     ...
     *
     * @return The sample media format, or null if a MOOV atom has yet to be
     *         parsed.
     */
     ...
     * <p>
     * The read terminates if the end of the input stream is reached, if an
     * attempt to read from the input stream returned 0 bytes of data, or if a
     * sample is read. The returned flags indicate both the reason for
     * termination and data that was parsed during the read.
     *
     * @param inputStream
     *            The input stream from which data should be read.
     * @param out
     *            A {@link SampleHolder} into which the next sample should be
     *            read. If null then {@link #RESULT_NEED_SAMPLE_HOLDER} will be
     *            returned once a sample has been reached.
     * @return One or more of the {@code RESULT_*} flags defined in this class.
     * @throws ParserException
     *             If an error occurs parsing the media data.
     */
     ...
     *
     * @param seekTimeUs
     *            The desired seek time in microseconds.
     * @param allowNoop
     *            Allow the seek operation to do nothing if the seek time is in
     *            the current fragment run, is equal to or greater than the time
     *            of the current sample, and if there does not exist a sync
     *            frame between these two times.
     * @return True if the operation resulted in a change of state. False if it
     *         was a no-op.
     */
    public boolean seekTo(long seekTimeUs, boolean allowNoop)
    {
        pendingSeekTimeMs = (int) (seekTimeUs / 1000);
        if (allowNoop && fragmentRun != null && pendingSeekTimeMs >= fragmentRun.getSamplePresentationTime(0) && pendingSeekTimeMs <= fragmentRun.getSamplePresentationTime(fragmentRun.length - 1))
        {
    ...
    private int onContainerAtomRead(ContainerAtom container)
    {
        if (container.type == Atom.TYPE_moov)
        {
            ...
            if (container.type == Atom.TYPE_moof)
            {
    ...
    private static Track parseTrak(ContainerAtom trak)
    {
        ...
        int id = header.first;
        // TODO: This value should be used to set a duration field on the Track
        // object
        // instantiated below, however we've found examples where the value is
        // 0. Revisit whether we
        // should set it anyway (and just have it be wrong for bad media
        // streams).
     ...
     *
     * @param mdhd
     *            The mdhd atom to parse.
     * @return The media timescale, defined as the number of time units that
     *         pass in one second.
     */
    ...
    private static Pair<MediaFormat, TrackEncryptionBox[]> parseStsd(ParsableByteArray stsd)
    {
        ...
        for (int i = 0; i < numberOfEntries; i++)
        {
            ...
            if (childAtomType == Atom.TYPE_avc1 || childAtomType == Atom.TYPE_avc3 || childAtomType == Atom.TYPE_encv)
            {
                ...
                if (childAtomType == Atom.TYPE_mp4a || childAtomType == Atom.TYPE_enca)
                {
    ...
    private static Pair<MediaFormat, TrackEncryptionBox> parseMp4aFromParent(ParsableByteArray parent, int position, int size)
    {
        ...
        while (childPosition - position < size)
        {
            ...
            if (childAtomType == Atom.TYPE_esds)
            {
                ...
                // Update sampleRate and channelCount from the
                // AudioSpecificConfig initialization data.
    ...
    private static List<byte[]> parseAvcCFromParent(ParsableByteArray parent, int position)
    {
        ...
        // TODO: We should try and parse these using
        // CodecSpecificDataUtil.parseSpsNalUnit, and
        // expose the AVC profile and level somewhere useful; Most likely in
        // MediaFormat.
    ...
    private static TrackEncryptionBox parseSinfFromParent(ParsableByteArray parent, int position, int size)
    {
        ...
        while (childPosition - position < size)
        {
            ...
            if (childAtomType == Atom.TYPE_frma)
            {
                ...
                if (childAtomType == Atom.TYPE_schm)
                {
                    parent.skip(4);
                    ...
                    if (childAtomType == Atom.TYPE_schi)
                    {
     ...
     *
     * @param extendsDefaults
     *            Default sample values from the trex atom.
    ...
    private static DefaultSampleValues parseTfhd(DefaultSampleValues extendsDefaults, ParsableByteArray tfhd)
    {
        ...
        int defaultSampleDescriptionIndex = ((flags & 0x02 /* default_sample_description_index_present */) != 0) ? tfhd.readUnsignedIntToInt() - 1 : extendsDefaults.sampleDescriptionIndex;
        int defaultSampleDuration = ((flags & 0x08 /* default_sample_duration_present */) != 0) ? tfhd.readUnsignedIntToInt() : extendsDefaults.duration;
        int defaultSampleSize = ((flags & 0x10 /* default_sample_size_present */) != 0) ? tfhd.readUnsignedIntToInt() : extendsDefaults.size;
        int defaultSampleFlags = ((flags & 0x20 /* default_sample_flags_present */) != 0) ? tfhd.readUnsignedIntToInt() : extendsDefaults.flags;
        return new DefaultSampleValues(defaultSampleDescriptionIndex, defaultSampleDuration, defaultSampleSize, defaultSampleFlags);
    }
     ...
     *
     * @return baseMediaDecodeTime. The sum of the decode durations of all
     *         earlier samples in the media, expressed in the media's timescale.
     */
     ...
     * @param defaultSampleValues
     *            Default sample values.
     * @param decodeTime
     *            The decode time.
     * @param trun
     *            The trun atom to parse.
     * @param out
     *            The {@TrackFragment} into which parsed data
     *            should be placed.
     */
    private static void parseTrun(Track track, DefaultSampleValues defaultSampleValues, long decodeTime, int workaroundFlags, ParsableByteArray trun, TrackFragment out)
    {
        ...
        boolean sampleCompositionTimeOffsetsPresent = (flags & 0x800 /* sample_composition_time_offsets_present */) != 0;
        ...
        boolean workaroundEveryVideoFrameIsSyncFrame = track.type == Track.TYPE_VIDEO && ((workaroundFlags & WORKAROUND_EVERY_VIDEO_FRAME_IS_SYNC_FRAME) == WORKAROUND_EVERY_VIDEO_FRAME_IS_SYNC_FRAME);
        for (int i = 0; i < sampleCount; i++)
        {
            ...
            {
                // The BMFF spec (ISO 14496-12) states that sample offsets
                // should be unsigned integers in
                // version 0 trun boxes, however a significant number of streams
                // violate the spec and use
                // signed integers instead. It's safe to always parse sample
                // offsets as signed integers
                // here, because unsigned integers will still be parsed
                // correctly (unless their top bit is
                // set, which is never true in practice because sample offsets
                // are always small).
    ...
    private static void parseUuid(ParsableByteArray uuid, TrackFragment out, byte[] extendedTypeScratch)
    {
        ...
        // Currently this parser only supports Microsoft's PIFF
        // SampleEncryptionBox.
        ...
        // Except for the extended type, this box is identical to a SENC box.
        // See "Portable encoding of
        // audio-video objects: The Protected Interoperable File Format (PIFF),
        // John A. Bocharov et al,
        // Section 5.3.2.1."
    ...
    private static SegmentIndex parseSidx(ParsableByteArray atom)
    {
        ...
        for (int i = 0; i < referenceCount; i++)
        {
            ...
            // Calculate time and duration values such that any rounding errors
            // are consistent. i.e. That
     ...
     * <p>
     * If there are no more samples in the current mdat atom then the parser
     * state is transitioned to {@link #STATE_READING_ATOM_HEADER} and 0 is
     * returned.
     * <p>
     * If there's a pending seek to a sync frame, and if the next sample is
     * before that frame, then the sample is skipped. Otherwise it is read.
     * <p>
     * It is possible for a sample to be read or skipped in part if there is
     * insufficent data available from the {@link NonBlockingInputStream}. In
     * this case the remainder of the sample can be read in a subsequent call
     * passing the same {@link SampleHolder}.
     *
     * @param inputStream
     *            The stream from which to read the sample.
     * @param out
     *            The holder into which to write the sample.
     * @return A combination of RESULT_* flags indicating the result of the
     *         call.
     */
    ...
    private int readSample(NonBlockingInputStream inputStream, int sampleSize, SampleHolder out)
    {
        ...
        else
        {
            ...
            {
                // The mp4 file contains length-prefixed NAL units, but the
                // decoder wants start code
    ...
    private void readSampleEncryptionData(ParsableByteArray sampleEncryptionData, SampleHolder out)
    {
        ...
        out.cryptoInfo.set(subsampleCount, clearDataSizes, encryptedDataSizes, keyId, vector, isEncrypted ? MediaCodec.CRYPTO_MODE_AES_CTR : MediaCodec.CRYPTO_MODE_UNENCRYPTED);
        if (isEncrypted)
        {
    ...
    /**
     * Parses the version number out of the additional integer component of a
     * full atom.
     */
    ...
    /**
     * Parses the atom flags out of the additional integer component of a full
     * atom.
     */
    ...
    /**
     * Reads an unsigned integer into an integer. This method is suitable for
     * use when it can be assumed that the top bit will always be set to zero.
     *
     * @throws IllegalArgumentException
     *             If the top bit of the input data is set.
     */
