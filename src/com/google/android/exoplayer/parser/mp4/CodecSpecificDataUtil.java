...
/**
 * Provides static utility methods for manipulating various types of codec
 * specific data.
 */
public final class CodecSpecificDataUtil
{
    ...
    /**
     * Builds a simple HE-AAC LC AudioSpecificConfig, as defined in ISO 14496-3
     * 1.6.2.1
     *
     * @param sampleRate
    ...
    public static byte[] buildAudioSpecificConfig(int sampleRate, int numChannels)
    {
        ...
        // The full specification for AudioSpecificConfig is stated in ISO
        // 14496-3 Section 1.6.2.1
        byte[] csd = new byte[2];
    ...
    /**
     * Constructs a NAL unit consisting of the NAL start code followed by the
     * specified data.
     *
     * @param data
     *            An array containing the data that should follow the NAL start
     *            code.
     * @param offset
     *            The start offset into {@code data}.
     * @param length
     *            The number of bytes to copy from {@code data}
     ...
     * <p>
     * If the input consists of NAL start code delimited units, then the
     * returned array consists of the split NAL units, each of which is still
     * prefixed with the NAL start code. For any other input, null is returned.
     *
     * @param data
     *            An array of data.
     * @return The individual NAL units, or null if the input did not consist of
     *         NAL start code delimited units.
     */
     ...
     * @param spsNalUnit
     *            The NAL unit.
     * @return A pair consisting of AVC profile and level constants, as defined
     *         in {@link CodecProfileLevel}. Null if the input data was not an
     *         SPS NAL unit.
     */
    public static Pair<Integer, Integer> parseSpsNalUnit(byte[] spsNalUnit)
    {
        ...
        // - Level idx (8 bits)
        if (isNalStartCode(spsNalUnit, 0) && spsNalUnit.length == 8 && (spsNalUnit[5] & 0x1F) == SPS_NAL_UNIT_TYPE)
        {
