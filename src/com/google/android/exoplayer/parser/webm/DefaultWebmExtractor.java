...
/**
 * Default version of an extractor to facilitate data retrieval from the WebM
 * container format.
 *
 * <p>
 * WebM is a subset of the EBML elements defined for Matroska. More information
 * about EBML and Matroska is available <a
 * href="http://www.matroska.org/technical/specs/index.html">here</a>. More info
 * about WebM is <a
 * href="http://www.webmproject.org/code/specs/container/">here</a>.
 */
@TargetApi(16)
public final class DefaultWebmExtractor implements WebmExtractor
{
    ...
    public int read(NonBlockingInputStream inputStream, SampleHolder sampleHolder)
    {
        ...
        while ((readResults & READ_TERMINATING_RESULTS) == 0)
        {
            ...
            if (ebmlReadResult == EbmlReader.READ_RESULT_NEED_MORE_DATA)
            {
                ...
                if (ebmlReadResult == EbmlReader.READ_RESULT_END_OF_STREAM)
                {
    ...
    {
        if (allowNoop && cues != null && clusterTimecodeUs != UNKNOWN && simpleBlockTimecodeUs != UNKNOWN && seekTimeUs >= simpleBlockTimecodeUs)
        {
    ...
    /* package */boolean onMasterElementStart(int id, long elementOffsetBytes, int headerSizeBytes, long contentsSizeBytes)
    {
        switch (id)
        {
        case ID_SEGMENT:
    ...
    /* package */boolean onIntegerElement(int id, long value)
    {
        ...
        {
        case ID_EBML_READ_VERSION:
            // Validate that EBMLReadVersion is supported. This extractor only
            // supports v1.
            if (value != 1)
            {
            ...
            break;
        case ID_DOC_TYPE_READ_VERSION:
            // Validate that DocTypeReadVersion is supported. This extractor
            // only supports up to v2.
            if (value < 1 || value > 2)
            {
    ...
    /* package */boolean onStringElement(int id, String value)
    {
        ...
        {
        case ID_DOC_TYPE:
            // Validate that DocType is supported. This extractor only supports
            // "webm".
            ...
            break;
        case ID_CODEC_ID:
            // Validate that CodecID is supported. This extractor only supports
            // "V_VP9".
    ...
    /* package */boolean onBinaryElement(int id, long elementOffsetBytes, int headerSizeBytes, int contentsSizeBytes, NonBlockingInputStream inputStream)
    {
        if (id == ID_SIMPLE_BLOCK)
        {
            ...
            int timecode = (simpleBlockTimecodeAndFlags[0] << 8) | (simpleBlockTimecodeAndFlags[1] & 0xff);
            ...
            if (sampleHolder.allowDataBufferReplacement && (sampleHolder.data == null || sampleHolder.data.capacity() < sampleHolder.size))
            {
    ...
    /**
     * Build a video {@link MediaFormat} containing recently gathered Video
     * information, if needed.
     *
     * <p>
     * Replaces the previous {@link #format} only if video width/height have
     * changed. {@link #format} is guaranteed to not be null after calling this
     * method. In the event that it can't be built, an
     * {@link IllegalStateException} will be thrown.
     */
    private void buildFormat()
    {
        if (pixelWidth != UNKNOWN && pixelHeight != UNKNOWN && (format == null || format.width != pixelWidth || format.height != pixelHeight))
        {
            format = MediaFormat.createVideoFormat(MimeTypes.VIDEO_VP9, MediaFormat.NO_VALUE, pixelWidth, pixelHeight, null);
    ...
    /**
     * Build a {@link SegmentIndex} containing recently gathered Cues
     * information.
     *
     * <p>
     * {@link #cues} is guaranteed to not be null after calling this method. In
     * the event that it can't be built, an {@link IllegalStateException} will
     * be thrown.
     */
    private void buildCues()
    {
        if (segmentStartOffsetBytes == UNKNOWN)
        {
            ...
            if (durationUs == UNKNOWN)
            {
                throw new IllegalStateException("Duration unknown");
            }
            else
                if (cuesSizeBytes == UNKNOWN)
                {
                    throw new IllegalStateException("Cues size unknown");
                }
                else
                    if (cueTimesUs == null || cueClusterPositions == null || cueTimesUs.size() == 0 || cueTimesUs.size() != cueClusterPositions.size())
                    {
    ...
    /**
     * Passes events through to {@link DefaultWebmExtractor} as callbacks from
     * {@link EbmlReader} are received.
     */
    private final class InnerEbmlEventHandler implements EbmlEventHandler
    {
        ...
        @Override
        public void onMasterElementStart(int id, long elementOffsetBytes, int headerSizeBytes, long contentsSizeBytes)
        {
            DefaultWebmExtractor.this.onMasterElementStart(id, elementOffsetBytes, headerSizeBytes, contentsSizeBytes);
        }
