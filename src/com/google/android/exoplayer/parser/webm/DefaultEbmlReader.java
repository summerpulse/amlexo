...
/**
 * Default version of a basic event-driven incremental EBML parser which needs
 * an {@link EbmlEventHandler} to define IDs/types and react to events.
 *
 * <p>
 * EBML can be summarized as a binary XML format somewhat similar to Protocol
 * Buffers. It was originally designed for the Matroska container format. More
 * information about EBML and Matroska is available <a
 * href="http://www.matroska.org/technical/specs/index.html">here</a>.
 */
/* package */final class DefaultEbmlReader implements EbmlReader
{

    // State values used in variables state, elementIdState,
    // elementContentSizeState, and
    // varintBytesState.
    ...
    /**
     * The first byte of a variable-length integer (varint) will have one of
     * these bit masks indicating the total length in bytes.
     *
     * <p>
     * {@code 0x80} is a one-byte integer, {@code 0x40} is two bytes, and so on
     * up to eight bytes.
     */
    ...
    /**
     * Scratch space to read in EBML varints, unsigned ints, and floats - each
     * of which can be up to 8 bytes.
     */
    private final byte[] tempByteArray = new byte[8];
    private final Stack<MasterElement> masterElementsStack = new Stack<MasterElement>();

    /**
     * Current {@link EbmlEventHandler} which is queried for element types and
     * informed of element events.
     */
    private EbmlEventHandler eventHandler;

    /**
     * Overall state for the current element. Must be one of the {@code STATE_*}
     * constants.
     */
    private int state;
    ...
    /**
     * Holds the current element ID after {@link #elementIdState} is
     * {@link #STATE_FINISHED_READING}.
     */
    private int elementId;

    /**
     * State for the ID of the current element. Must be one of the
     * {@code STATE_*} constants.
     */
    private int elementIdState;

    /**
     * Holds the current element content size after
     * {@link #elementContentSizeState} is {@link #STATE_FINISHED_READING}.
     */
    private long elementContentSize;

    /**
     * State for the content size of the current element. Must be one of the
     * {@code STATE_*} constants.
     */
    ...
    /**
     * Length in bytes of the current variable-length integer (varint) being
     * read into {@link #tempByteArray}.
     */
    private int varintBytesLength;

    /**
     * Counts the number of bytes being contiguously read into either
     * {@link #tempByteArray} or {@link #stringBytes}. Used to determine when
     * all required bytes have been read across multiple calls.
     */
    private int bytesState;

    /**
     * Holds string element bytes as they're being read in. Allocated after the
     * element content size is known and released after calling
     * {@link EbmlEventHandler#onStringElement(int, String)}.
     */
    ...
    public int read(NonBlockingInputStream inputStream)
    {
        ...
        while (true)
        {
            while (!masterElementsStack.isEmpty() && bytesRead >= masterElementsStack.peek().elementEndOffsetBytes)
            {
            ...
            {
            case TYPE_MASTER:
                int masterHeaderSize = (int) (bytesRead - elementOffset); // Header
                                                                          // size
                                                                          // is
                                                                          // 12
                                                                          // bytes
                ...
                eventHandler.onMasterElementStart(elementId, elementOffset, masterHeaderSize, elementContentSize);
            ...
            case TYPE_STRING:
                ...
                {
                    throw new IllegalStateException("String element size " + elementContentSize + " is larger than MAX_INT");
                }
                if (stringBytes == null)
                {
            ...
            case TYPE_BINARY:
                if (elementContentSize > Integer.MAX_VALUE)
                {
                    throw new IllegalStateException("Binary element size " + elementContentSize + " is larger than MAX_INT");
                }
                if (inputStream.getAvailableByteCount() < elementContentSize)
                {
                    return READ_RESULT_NEED_MORE_DATA;
                }
                int binaryHeaderSize = (int) (bytesRead - elementOffset); // Header
                                                                          // size
                                                                          // is
                                                                          // 12
                                                                          // bytes
                                                                          // max.
                boolean consumed = eventHandler.onBinaryElement(elementId, elementOffset, binaryHeaderSize, (int) elementContentSize, inputStream);
                if (consumed)
                {
                    long expectedBytesRead = elementOffset + binaryHeaderSize + elementContentSize;
                    if (expectedBytesRead != bytesRead)
                    {
                        throw new IllegalStateException("Incorrect total bytes read. Expected " + expectedBytesRead + " but actually " + bytesRead);
                    }
                    prepareForNextElement();
                }
                return READ_RESULT_CONTINUE;
            case TYPE_UNKNOWN:
                if (elementContentSize > Integer.MAX_VALUE)
                {
                    throw new IllegalStateException("Unknown element size " + elementContentSize + " is larger than MAX_INT");
                }
    ...
    /**
     * Resets the internal state of {@link #read(NonBlockingInputStream)} so
     * that it can start reading a new element from scratch.
     */
    ...
    /**
     * Reads an element ID such that reading can be stopped and started again in
     * a later call if not enough bytes are available. Returns
     * {@link #READ_RESULT_CONTINUE} if a full element ID has been read into
     * {@link #elementId}. Reset {@link #elementIdState} to
     * {@link #STATE_BEGIN_READING} before calling to indicate a new element ID
     * should be read.
     *
     * @param inputStream
     *            The input stream from which an element ID should be read
    ...
    /**
     * Reads an element's content size such that reading can be stopped and
     * started again in a later call if not enough bytes are available.
     *
     * <p>
     * Returns {@link #READ_RESULT_CONTINUE} if an entire element size has been
     * read into {@link #elementContentSize}. Reset
     * {@link #elementContentSizeState} to {@link #STATE_BEGIN_READING} before
     * calling to indicate a new element size should be read.
     *
     * @param inputStream
     *            The input stream from which an element size should be read
     * @return One of the {@code RESULT_*} flags defined in this class
     */
    ...
    /**
     * Reads an EBML variable-length integer (varint) such that reading can be
     * stopped and started again in a later call if not enough bytes are
     * available.
     *
     * <p>
     * Returns {@link #READ_RESULT_CONTINUE} if an entire varint has been read
     * into {@link #tempByteArray} and the length of the varint is in
     * {@link #varintBytesLength}. Reset {@link #varintBytesState} to
     * {@link #STATE_BEGIN_READING} before calling to indicate a new varint
     * should be read.
     *
     * @param inputStream
     *            The input stream from which a varint should be read
     * @return One of the {@code RESULT_*} flags defined in this class
     */
    private int readVarintBytes(NonBlockingInputStream inputStream)
    {
        ...
        if (varintBytesState == STATE_BEGIN_READING)
        {
            ...
            {
                throw new IllegalStateException("No valid varint length mask found at bytesRead = " + bytesRead);
            }
        }
    ...
    /**
     * Reads a set amount of bytes into a {@link ByteBuffer} such that reading
     * can be stopped and started again later if not enough bytes are available.
     *
     * <p>
     * Returns {@link #READ_RESULT_CONTINUE} if all bytes have been read. Reset
     * {@link #bytesState} to {@code 0} before calling to indicate a new set of
     * bytes should be read.
     *
     * @param inputStream
     *            The input stream from which bytes should be read
     * @param byteBuffer
     *            The {@link ByteBuffer} into which bytes should be read
     * @param totalBytes
     *            The total size of bytes to be read
     * @return One of the {@code RESULT_*} flags defined in this class
     */
    ...
    /**
     * Reads a set amount of bytes into a {@code byte[]} such that reading can
     * be stopped and started again later if not enough bytes are available.
     *
     * <p>
     * Returns {@link #READ_RESULT_CONTINUE} if all bytes have been read. Reset
     * {@link #bytesState} to {@code 0} before calling to indicate a new set of
     ...
     *            The input stream from which bytes should be read
     * @param byteArray
     *            The {@code byte[]} into which bytes should be read
     * @param totalBytes
     *            The total size of bytes to be read
     * @return One of the {@code RESULT_*} flags defined in this class
     */
    ...
    /**
     * Skips a set amount of bytes such that reading can be stopped and started
     * again later if not enough bytes are available.
     *
     * <p>
     * Returns {@link #READ_RESULT_CONTINUE} if all bytes have been skipped.
     * Reset {@link #bytesState} to {@code 0} before calling to indicate a new
     * set of bytes should be skipped.
     *
     * @param inputStream
     *            The input stream from which bytes should be skipped
     * @param totalBytes
     *            The total size of bytes to be skipped
     * @return One of the {@code RESULT_*} flags defined in this class
     */
    ...
    /**
     * Updates {@link #bytesState} and {@link #bytesRead} after reading bytes in
     * one of the {@code verbBytesInternal} methods.
     *
     * @param additionalBytesRead
     *            The number of additional bytes read to be accounted for
     * @param totalBytes
     *            The total size of bytes to be read or skipped
     * @return One of the {@code RESULT_*} flags defined in this class
     */
    ...
    /**
     * Parses and returns the integer value currently read into the first
     * {@code byteLength} bytes of {@link #tempByteArray}. EBML varint length
     * masks can optionally be removed.
     *
     * @param byteLength
     *            The number of bytes to parse from {@link #tempByteArray}
     * @param removeLengthMask
     *            Removes the variable-length integer length mask from the value
     * @return The resulting integer value. This value could be up to 8-bytes so
     *         a Java long is used
     */
    private long getTempByteArrayValue(int byteLength, boolean removeLengthMask)
    {
        ...
        {
            // Shift all existing bits up one byte and add the next byte at the
            // bottom.
    ...
    /**
     * Used in {@link #masterElementsStack} to track when the current master
     * element ends so that {@link EbmlEventHandler#onMasterElementEnd(int)} is
     * called.
     */
