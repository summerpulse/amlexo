...
public class MediaCodecAudioTrackRenderer extends MediaCodecTrackRenderer
{
     ...
     * {@link MediaCodecAudioTrackRenderer} events.
     */
     ...
     * {@link ExoPlayer#sendMessage} or {@link ExoPlayer#blockingSendMessage}.
     * The message object should be a {@link Float} with 0 being silence and 1
     * being unity gain.
     */
    public static final int MSG_SET_VOLUME = 1;

    /**
     * The default multiplication factor used when determining the size of the
     * underlying {@link AudioTrack}'s buffer.
     */
    ...
    /**
     * @param source
     *            The upstream source from which the renderer obtains samples.
     */
     ...
     * @param source
     *            The upstream source from which the renderer obtains samples.
     * @param drmSessionManager
     *            For use with encrypted content. May be null if support for
     *            encrypted content is not required.
     * @param playClearSamplesWithoutKeys
     *            Encrypted media may contain clear (un-encrypted) regions. For
     *            example a media file may start with a short clear region so as
     *            to allow playback to begin in parallel with key acquisision.
     *            This parameter specifies whether the renderer is permitted to
     *            play clear regions of encrypted media files before
     *            {@code drmSessionManager} has obtained the keys necessary to
     *            decrypt encrypted regions of the media.
     */
     ...
     *            {@code eventListener}. May be null if delivery of events is
     *            not required.
     * @param eventListener
     *            A listener of events. May be null if delivery of events is not
     *            required.
     */
    ...
    {
        this(source, drmSessionManager, playClearSamplesWithoutKeys, DEFAULT_MIN_BUFFER_MULTIPLICATION_FACTOR, eventHandler, eventListener);
    }

    /**
     * @param source
     *            The upstream source from which the renderer obtains samples.
     * @param minBufferMultiplicationFactor
     *            When instantiating an underlying {@link AudioTrack}, the size
     *            of the track's is calculated as this value multiplied by the
     *            minimum buffer size obtained from
     *            {@link AudioTrack#getMinBufferSize(int, int, int)}. The
     *            multiplication factor must be greater than or equal to 1.
     * @param eventHandler
     *            A handler to use when delivering events to
     *            {@code eventListener}. May be null if delivery of events is
     *            not required.
    ...
    protected void onOutputFormatChanged(MediaFormat format)
    {
        ...
        if (audioTrack != null && this.sampleRate == sampleRate && this.channelConfig == channelConfig)
        {
            // We already have an existing audio track with the correct sample
            // rate and channel config.
            return;
        }
        ...
        this.minBufferSize = AudioTrack.getMinBufferSize(sampleRate, channelConfig, AudioFormat.ENCODING_PCM_16BIT);
        this.bufferSize = (int) (minBufferMultiplicationFactor * minBufferSize);
        this.frameSize = 2 * channelCount; // 2 bytes per 16 bit sample * number
                                           // of channels.
    }

    private void initAudioTrack() throws ExoPlaybackException
    {
        // If we're asynchronously releasing a previous audio track then we
        // block until it has been
        // released. This guarantees that we cannot end up in a state where we
        // have multiple audio
        // track instances. Without this guarantee it would be possible, in
        // extreme cases, to exhaust
        // the shared memory that's available for audio track buffers. This
        ...
        else
        {
            ...
            audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, sampleRate, channelConfig, AudioFormat.ENCODING_PCM_16BIT, bufferSize, AudioTrack.MODE_STREAM, audioSessionId);
            checkAudioTrackInitialized();
        }
    ...
    /**
     * Checks that {@link #audioTrack} has been successfully initialized. If it
     * has then calling this method is a no-op. If it hasn't then
     * {@link #audioTrack} is released and set to null, and an exception is
     * thrown.
     *
     * @throws ExoPlaybackException
     *             If {@link #audioTrack} has not been successfully initialized.
     */
    private void checkAudioTrackInitialized() throws ExoPlaybackException
    {
        ...
        // The track is not successfully initialized. Release and null the
        // track.
        try
        {
            audioTrack.release();
        ...
        {
            // The track has already failed to initialize, so it wouldn't be
            // that surprising if release
        ...
        AudioTrackInitializationException exception = new AudioTrackInitializationException(audioTrackState, sampleRate, channelConfig, bufferSize);
        notifyAudioTrackInitializationError(exception);
        throw new ExoPlaybackException(exception);
    }

    /**
     * Invoked when the audio session id becomes known. Once the id is known it
     * will not change (and hence this method will not be invoked again) unless
     * the renderer is disabled and then subsequently re-enabled.
     * <p>
     * The default implementation is a no-op. One reason for overriding this
     * method would be to instantiate and enable a
     * {@link android.media.audiofx.Virtualizer} in order to spatialize the
     * audio channels. For this use case, any
     * {@link android.media.audiofx.Virtualizer} instances should be released in
    ...
    private void releaseAudioTrack()
    {
        if (audioTrack != null)
        {
            ...
            // AudioTrack.release can take some time, so we call it on a
            // background thread.
    ...
    {
        // We've exhausted the output stream, and the AudioTrack has either
     ...
     *
     * 1. Prior to playback having started, calls up to the super class to
     * obtain the pending seek position. 2. During playback, uses
     * AudioTimestamps obtained from AudioTrack.getTimestamp on supported
     * devices. 3. Else, derives a smoothed position by sampling the
     * AudioTrack's frame position.
     */
    @Override
    protected long getCurrentPositionUs()
    {
        ...
        if (audioTrack == null || audioTrackStartMediaTimeState == START_NOT_SET)
        {
            ...
            {
                // How long ago in the past the audio timestamp is (negative if
                // it's in the future)
            ...
            else
            {
                ...
                {
                    // The AudioTrack has started, but we don't have any samples
                    // to compute a smoothed position.
                    currentPositionUs = getPlayheadPositionUs() + audioTrackStartMediaTimeUs;
                }
                else
                {
                    // getPlayheadPositionUs() only has a granularity of ~20ms,
                    // so we base the position off the
                    // system clock (and a smoothed offset between it and the
                    // playhead position) so as to
        ...
        // Make sure we don't ever report time moving backwards as a result of
        // smoothing or switching
    ...
    {
        if (audioTrack == null || audioTrackStartMediaTimeState == START_NOT_SET || getState() != STATE_STARTED)
        {
        ...
        {
            // Take a new sample and update the smoothed offset between the
            // system clock and the playhead.
        ...
        if (systemClockUs - lastTimestampSampleTimeUs >= MIN_TIMESTAMP_SAMPLE_INTERVAL_US)
        {
            ...
            if (audioTimestampSet && (audioTimestampCompat.getNanoTime() / 1000) < audioTrackResumeSystemTimeUs)
            {
                // The timestamp was set, but it corresponds to a time before
                // the track was most recently
                // resumed.
            ...
            if (audioTrackGetLatencyMethod != null)
            {
                try
                {
                    // Compute the audio track latency, excluding the latency
                    // due to the buffer (leaving
                    // latency due to the mixer and audio hardware driver).
                    audioTrackLatencyUs = (Integer) audioTrackGetLatencyMethod.invoke(audioTrack, (Object[]) null) * 1000L - framesToDurationUs(bufferSize / frameSize);
    ...
    {
        super.seekTo(timeUs);
        // TODO: Try and re-use the same AudioTrack instance once [redacted] is
        // fixed.
        releaseAudioTrack();
        lastReportedCurrentPositionUs = 0;
    }
    ...
    protected boolean processOutputBuffer(long timeUs, MediaCodec codec, ByteBuffer buffer, MediaCodec.BufferInfo bufferInfo, int bufferIndex, boolean shouldSkip) throws ExoPlaybackException
    {
        if (shouldSkip)
        {
            ...
            {
                // Skipping the sample will push track time out of sync. We'll
                // need to sync again.
        ...
        if (temporaryBufferSize == 0)
        {
            ...
            // Note: presentationTimeUs corresponds to the end of the sample,
            // not the start.
            long bufferStartTime = bufferInfo.presentationTimeUs - framesToDurationUs(bufferInfo.size / frameSize);
            ...
            {
                // Sanity check that bufferStartTime is consistent with the
                // expected value.
                long expectedBufferStartTime = audioTrackStartMediaTimeUs + framesToDurationUs(submittedBytes / frameSize);
                if (audioTrackStartMediaTimeState == START_IN_SYNC && Math.abs(expectedBufferStartTime - bufferStartTime) > 200000)
                {
                ...
                {
                    // Adjust audioTrackStartMediaTimeUs to be consistent with
                    // the current buffer's start
                    // time and the number of bytes submitted. Also reset
                    // lastReportedCurrentPositionUs to
    ...
    /**
     * {@link AudioTrack#getPlaybackHeadPosition()} returns a value intended to
     * be interpreted as an unsigned 32 bit integer, which also wraps around
     * periodically. This method returns the playback head position as a long
     * that will only wrap around if the value exceeds {@link Long#MAX_VALUE}
     * (which in practice will never happen).
     *
     * @return {@link AudioTrack#getPlaybackHeadPosition()} of
     *         {@link #audioTrack} expressed as a long.
     */
    ...
    /**
     * Interface exposing the {@link AudioTimestamp} methods we need that were
     * added in SDK 19.
     */
    ...
    /**
     * The AudioTimestampCompat implementation for SDK < 19 that does nothing or
     * throws an exception.
     */
    ...
    /**
     * The AudioTimestampCompat implementation for SDK >= 19 that simply calls
     * through to the actual implementations added in SDK 19.
     */
    @TargetApi(19)
