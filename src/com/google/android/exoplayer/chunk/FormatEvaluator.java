...
public interface FormatEvaluator
{
     ...
     * <p>
     * When the method is invoked, {@code evaluation} will contain the currently
     * selected format (null for the first evaluation), the most recent trigger
     * (TRIGGER_INITIAL for the first evaluation) and the current queue size.
     * The implementation should update these fields as necessary.
     * <p>
     ...
     *
     * @param queue
     *            A read only representation of the currently buffered
     *            {@link MediaChunk}s.
     * @param playbackPositionUs
     *            The current playback position.
     * @param formats
     *            The formats from which to select, ordered by decreasing
     *            bandwidth.
     * @param evaluation
     *            The evaluation.
     */
    ...
    /**
     * An adaptive evaluator for video formats, which attempts to select the
     * best quality possible given the current network conditions and state of
     * the buffer.
     * <p>
     * This implementation should be used for video only, and should not be used
     * for audio. It is a reference implementation only. It is recommended that
     * application developers implement their own adaptive evaluator to more
     * precisely suit their use case.
     */
    public static class AdaptiveEvaluator implements FormatEvaluator
    {
        ...
        {
            this(bandwidthMeter, DEFAULT_MAX_INITIAL_BITRATE, DEFAULT_MIN_DURATION_FOR_QUALITY_INCREASE_MS, DEFAULT_MAX_DURATION_FOR_QUALITY_DECREASE_MS, DEFAULT_MIN_DURATION_TO_RETAIN_AFTER_DISCARD_MS, DEFAULT_BANDWIDTH_FRACTION);
        }

        /**
         * @param bandwidthMeter
         *            Provides an estimate of the currently available bandwidth.
         * @param maxInitialBitrate
         *            The maximum bitrate in bits per second that should be
         *            assumed when bandwidthMeter cannot provide an estimate due
         *            to playback having only just started.
         * @param minDurationForQualityIncreaseMs
         *            The minimum duration of buffered data required for the
         *            evaluator to consider switching to a higher quality
         *            format.
         * @param maxDurationForQualityDecreaseMs
         *            The maximum duration of buffered data required for the
         *            evaluator to consider switching to a lower quality format.
         * @param minDurationToRetainAfterDiscardMs
         *            When switching to a significantly higher quality format,
         *            the evaluator may discard some of the media that it has
         *            already buffered at the lower quality, so as to switch up
         *            to the higher quality faster. This is the minimum duration
         *            of media that must be retained at the lower quality.
         * @param bandwidthFraction
         *            The fraction of the available bandwidth that the evaluator
         *            should consider available for use. Setting to a value less
         *            than 1 is recommended to account for inaccuracies in the
         *            bandwidth estimator.
         */
        ...
        public void evaluate(List<? extends MediaChunk> queue, long playbackPositionUs, Format[] formats, Evaluation evaluation)
        {
            long bufferedDurationUs = queue.isEmpty() ? 0 : queue.get(queue.size() - 1).endTimeUs - playbackPositionUs;
            ...
            if (isHigher)
            {
                ...
                {
                    // The ideal format is a higher quality, but we have
                    // insufficient buffer to
                    ...
                    {
                        // We're switching from an SD stream to a stream of
                        // higher resolution. Consider
                        // discarding already buffered media chunks.
                        // Specifically, discard media chunks starting
                        // from the first one that is of lower bandwidth, lower
                        // resolution and that is not HD.
                        for (int i = 1; i < queue.size(); i++)
                        {
                            MediaChunk thisChunk = queue.get(i);
                            long durationBeforeThisSegmentUs = thisChunk.startTimeUs - playbackPositionUs;
                            if (durationBeforeThisSegmentUs >= minDurationToRetainAfterDiscardUs && thisChunk.format.bitrate < ideal.bitrate && thisChunk.format.height < ideal.height && thisChunk.format.height < 720 && thisChunk.format.width < 1280)
                            {
                ...
                if (isLower && current != null && bufferedDurationUs >= maxDurationForQualityDecreaseUs)
                {
                    // The ideal format is a lower quality, but we have
                    // sufficient buffer to defer switching
                    // down for now.
                    ideal = current;
                }
        ...
        protected Format determineIdealFormat(Format[] formats, long bitrateEstimate)
        {
            ...
            // We didn't manage to calculate a suitable format. Return the
        ...
        {
            return bitrateEstimate == BandwidthMeter.NO_ESTIMATE ? maxInitialBitrate : (long) (bitrateEstimate * bandwidthFraction);
        }
