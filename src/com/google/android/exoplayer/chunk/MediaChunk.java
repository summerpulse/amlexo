...
public abstract class MediaChunk extends Chunk
{
    ...
    /**
     * The index of the next media chunk, or -1 if this is the last media chunk
     * in the stream.
     */
     ...
     */
    public MediaChunk(DataSource dataSource, DataSpec dataSpec, Format format, int trigger, long startTimeUs, long endTimeUs, int nextChunkIndex)
    {
     ...
     * @param allowNoop
     *            True if the seek is allowed to do nothing if the result is
     *            more accurate than seeking to a key frame. Always pass false
     *            if it is required that the next sample be a key frame.
     * @return True if the seek results in a discontinuity in the sequence of
     *         samples returned by {@link #read(SampleHolder)}. False otherwise.
     */
    public abstract boolean seekTo(long positionUs, boolean allowNoop);

    /**
     * Prepares the chunk for reading. Does nothing if the chunk is already
     * prepared.
     * <p>
     * Preparation may require consuming some of the chunk. If the data is not
     * yet available then this method will return {@code false} rather than
     * block. The method can be called repeatedly until the return value
     * indicates success.
     *
     ...
     *
     * @return True if the next sample is available for reading. False
     *         otherwise.
     * @throws ParserException
     */
     ...
     *
     * @param holder
     *            A holder to store the read sample.
     * @return True if a sample was read. False if more data is still required.
     * @throws ParserException
     *             If an error occurs parsing the media data.
     * @throws IllegalStateException
     *             If called before {@link #init}, or after {@link #release}
     */
