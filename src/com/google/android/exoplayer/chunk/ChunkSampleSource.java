...
/**
 * A {@link SampleSource} that loads media in {@link Chunk}s, which are
 * themselves obtained from a {@link ChunkSource}.
 */
public class ChunkSampleSource implements SampleSource, Loader.Listener
{
    ...
    public interface EventListener
    {
         ...
         *
         * @param sourceId
         *            The id of the reporting {@link SampleSource}.
         * @param formatId
         *            The format id.
         * @param trigger
         *            A trigger for the format selection, as specified by the
         *            {@link ChunkSource}.
         * @param isInitialization
         *            Whether the load is for format initialization data.
         * @param mediaStartTimeMs
         *            The media time of the start of the data being loaded, or
         *            -1 if this load is for initialization data.
         * @param mediaEndTimeMs
         *            The media time of the end of the data being loaded, or -1
         *            if this load is for initialization data.
         * @param length
         *            The length of the data being loaded in bytes, or
         *            {@link C#LENGTH_UNBOUNDED} if the length of the data has
         ...
         *            The id of the reporting {@link SampleSource}.
         * @param bytesLoaded
         *            The number of bytes that were loaded prior to the
         *            cancellation.
         */
         ...
         * so that it can be re-buffered using a different representation.
         *
         * @param sourceId
         *            The id of the reporting {@link SampleSource}.
         * @param mediaStartTimeMs
         *            The media time of the start of the discarded data.
         * @param mediaEndTimeMs
         *            The media time of the end of the discarded data.
         * @param bytesDiscarded
        ...
        /**
         * Invoked when data is removed from the front of the buffer, typically
         * due to a seek or because the data has been consumed.
         *
         * @param sourceId
         *            The id of the reporting {@link SampleSource}.
         * @param mediaStartTimeMs
         ...
         * being supplied to the caller of {@link SampleSource#readData}
         * changes).
         *
         * @param sourceId
         *            The id of the reporting {@link SampleSource}.
         ...
         *            The format id.
         * @param trigger
         *            The trigger specified in the corresponding upstream load,
         *            as specified by the {@link ChunkSource}.
         * @param mediaTimeMs
    ...
    /**
     * Exposes the current downstream format for debugging purposes. Can be
    ...
    public boolean continueBuffering(long playbackPositionUs) throws IOException
    {
        ...
        if (isPendingReset() || mediaChunks.isEmpty())
        {
            ...
            if (mediaChunks.getFirst().sampleAvailable())
            {
            ...
            {
                // It may be the case that the current chunk has been fully read
                // but not yet discarded and
                // that the next chunk has an available sample. Return true if
                // so, otherwise false.
    ...
    private MediaChunk getMediaChunk(long timeUs)
    {
        ...
        while (mediaChunkIterator.hasNext())
        {
            ...
            if (timeUs < mediaChunk.startTimeUs)
            {
                ...
                if (mediaChunk.isLastChunk() || timeUs < mediaChunk.endTimeUs)
                {
                    return mediaChunk;
                }
        }
    ...
    /**
     * Called when a sample has been read from a {@link MediaChunk}. Can be used
     * to perform any modifications necessary before the sample is returned.
     *
     * @param mediaChunk
     *            The MediaChunk the sample was ready from.
     * @param sampleHolder
     *            The sample that has just been read.
     */
    ...
    private void updateLoadControl()
    {
        ...
        boolean nextLoader = loadControl.update(this, downstreamPositionUs, loadPositionUs, isBackedOff || loader.isLoading(), currentLoadableExceptionFatal);
        ...
        if (!loader.isLoading())
        {
            if (currentLoadableHolder.chunk == null || now - lastPerformedBufferOperation > 1000)
            {
                ...
                chunkSource.getChunkOperation(readOnlyMediaChunks, pendingResetTime, downstreamPositionUs, currentLoadableHolder);
     ...
     * Resumes loading.
     * <p>
     * If the {@link ChunkSource} returns a chunk equivalent to the backed off
     * chunk B, then the loading of B will be resumed. In all other cases B will
     * be discarded and the new chunk will be loaded.
     */
    private void resumeFromBackOff()
    {
        ...
        {
            // We're not able to clear the first media chunk, so we have no
        ...
        // The current loadable is the last media chunk. Remove it before we
        // invoke the chunk source,
        ...
        {
            // This call will remove and release at least one chunk from the end
            // of mediaChunks. Since
            // the current loadable is the last media chunk, it is guaranteed to
            // be removed.
            discardUpstreamMediaChunks(currentLoadableHolder.queueSize);
            clearCurrentLoadable();
            maybeStartLoading();
    ...
    private void maybeStartLoading()
    {
        ...
        {
            notifyLoadStarted(currentLoadable.format.id, currentLoadable.trigger, true, -1, -1, currentLoadable.getLength());
        }
        loader.startLoading(currentLoadable);
    }

    /**
     * Discards downstream media chunks until {@code untilChunk} if found.
     * {@code untilChunk} is not itself discarded. Null can be passed to discard
     * all media chunks.
     *
     * @param untilChunk
     *            The first media chunk to keep, or null to discard all media
     *            chunks.
     */
     ...
     *
     * @param queueLength
     *            The desired length of the queue.
     */
    ...
    private void notifyLoadStarted(final String formatId, final int trigger, final boolean isInitialization, final long mediaStartTimeUs, final long mediaEndTimeUs, final long length)
    {
        if (eventHandler != null && eventListener != null)
        {
            eventHandler.post(new Runnable()
            {
                ...
                public void run()
                {
                    eventListener.onLoadStarted(eventSourceId, formatId, trigger, isInitialization, usToMs(mediaStartTimeUs), usToMs(mediaEndTimeUs), length);
                }
            });
        }
    }
    ...
    private void notifyUpstreamDiscarded(final long mediaStartTimeUs, final long mediaEndTimeUs, final long totalBytes)
    {
        if (eventHandler != null && eventListener != null)
        {
            eventHandler.post(new Runnable()
            {
                ...
                public void run()
                {
                    eventListener.onUpstreamDiscarded(eventSourceId, usToMs(mediaStartTimeUs), usToMs(mediaEndTimeUs), totalBytes);
                }
            });
        }
    }
    ...
    private void notifyDownstreamFormatChanged(final String formatId, final int trigger, final long mediaTimeUs)
    {
        if (eventHandler != null && eventListener != null)
        {
            eventHandler.post(new Runnable()
            {
                ...
                public void run()
                {
                    eventListener.onDownstreamFormatChanged(eventSourceId, formatId, trigger, usToMs(mediaTimeUs));
                }
            });
        }
    }

    private void notifyDownstreamDiscarded(final long mediaStartTimeUs, final long mediaEndTimeUs, final long bytesDiscarded)
    {
        if (eventHandler != null && eventListener != null)
        {
            eventHandler.post(new Runnable()
            {
                ...
                public void run()
                {
                    eventListener.onDownstreamDiscarded(eventSourceId, usToMs(mediaStartTimeUs), usToMs(mediaEndTimeUs), bytesDiscarded);
                }
            });
        }
    }
