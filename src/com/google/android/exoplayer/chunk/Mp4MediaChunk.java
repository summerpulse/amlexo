...
public final class Mp4MediaChunk extends MediaChunk
{
     ...
     * @param maybeSelfContained
     *            Set to true if this chunk might be self contained, meaning it
     *            might contain a moov atom defining the media format of the
     *            chunk. This parameter can always be safely set to true.
     *            Setting to false where the chunk is known to not be self
     *            contained may improve startup latency.
     * @param sampleOffsetUs
     *            An offset to subtract from the sample timestamps parsed by the
     *            extractor.
     */
    ...
    public boolean prepare() throws ParserException
    {
        if (!prepared)
        {
            ...
            {
                // Read up to the first sample. Once we're there, we know that
            ...
            {
                // We know there isn't a moov atom. The extractor must have
