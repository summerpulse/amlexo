...
/* package */final class ExoPlayerImpl implements ExoPlayer
{
    ...
    /**
     * Constructs an instance. Must be invoked from a thread that has an
     * associated {@link Looper}.
     *
     * @param rendererCount
     *            The number of {@link TrackRenderer}s that will be passed to
     *            {@link #prepare(TrackRenderer[])}.
     * @param minBufferMs
     *            A minimum duration of data that must be buffered for playback
     *            to start or resume following a user action such as a seek.
     * @param minRebufferMs
     *            A minimum duration of data that must be buffered for playback
     *            to resume after a player invoked rebuffer (i.e. a rebuffer
     *            that occurs due to buffer depletion, and not due to a user
     *            action such as starting playback or seeking).
     */
    ...
    public ExoPlayerImpl(int rendererCount, int minBufferMs, int minRebufferMs)
    {
        ...
        internalPlayer = new ExoPlayerImplInternal(eventHandler, playWhenReady, rendererEnabledFlags, minBufferMs, minRebufferMs);
    }
    ...
    public int getBufferedPercentage()
    {
        ...
        return bufferedPosition == ExoPlayer.UNKNOWN_TIME || duration == ExoPlayer.UNKNOWN_TIME ? 0 : (duration == 0 ? 100 : (bufferedPosition * 100) / duration);
    }
